...
- **Adequate Infrastructure:** When infrastructure meets or exceeds the colony's needs, population grows at full rate. *(requires live testing)*
- **Sector Governor Bonus:** Governors (see [Section 16.1 Officer Generation](../16-commanders/16.1-officer-generation.md)) with population growth bonuses increase the rate for their assigned colony. *(requires live testing)*
- **Low Overcrowding:** When a colony is well below its carrying capacity, growth proceeds normally. *(requires live testing)*

**Negative Modifiers:**

- **Insufficient Infrastructure:** If infrastructure is below the required level for the current population and colony cost, growth is reduced. Severe shortfalls can cause population decline (deaths from life support failure). *(requires live testing)*
- **Overcrowding:** When population exceeds the colony's comfortable capacity, growth slows. *(requires live testing)*
- **Environmental Damage:** Radiation, extreme temperatures without adequate infrastructure, or toxic atmosphere breaches slow growth. *(requires live testing)*
- **Lack of Employment:** Extreme unemployment can reduce growth rates as economic conditions deteriorate. *(requires live testing)*

### 5.2.1.3 Population Decline

Population can decrease in several situations:

- **Infrastructure Failure:** If infrastructure drops below critical levels (due to maintenance failure, enemy bombardment, or transportation issues), population will die. *(requires live testing)*
- **Orbital Bombardment:** Enemy attacks on a colony directly kill population (see [Section 12.1 Fire Controls](../12-combat/12.1-fire-controls.md)). *(requires live testing)*
- **Environmental Catastrophe:** Loss of atmosphere containment on hostile worlds. *(requires live testing)*
- **Emigration:** Population may leave a colony for better opportunities elsewhere (primarily a civilian economy mechanic). *(requires live testing)*
...
Not all population is available for manufacturing and mining work. Approximately 60% of total population is available as workers (the remainder being children, elderly, and service sector). *(requires live testing -- percentage not stored in database)* Of that workforce, a portion is further allocated to agriculture and environment based on the world's colony cost.
...
**Naval Shipyard Workers:** Shipyard worker requirements are calculated separately based on shipyard size (250 workers per ton of shipyard capacity for naval shipyards). *(requires live testing -- formula not stored in installation table)*
...
| **Initial Population Cost** | Consumes 100,000 population upon construction *(requires live testing)* |
| **Ongoing Workers** | 5,000 population needed as overseers \hyperlink{ref-5.2-6}{[6]} |
| **Unrest Penalty** | +5 points of unrest instantly when built *(requires live testing)* |
...
**Formula:** ((Determination + Militancy + Xenophobia) / 300) x Population Amount x Political Status Modifier *(community-derived formula -- requires live testing)*
...
(Square Root of Size x Units x Morale) / 10,000 *(community-derived formula -- requires live testing)*
...
4. **Unrest Reduction Rate** = 100 x Police Modifier (points per year) *(requires live testing)*
...
- **High unrest (40+):** Production penalties begin, with output numbers visibly declining *(requires live testing)*
