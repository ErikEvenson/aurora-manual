...
![](../images/art/section-art/5.2.png)
...
![Population Growth Rate Comparison](../images/charts/5-colonies/5.2-population-growth-comparison.png)
...
- **Sector Governor Bonus:** Governors (see [Section 16.1 Officer Generation](../16-commanders/16.1-officer-generation.md)) with population growth bonuses increase the rate for their assigned colony. *(requires live testing — #736)*
...
- **Orbital Bombardment:** Enemy attacks on a colony directly kill population (see [Section 12.1 Fire Controls](../12-combat/12.1-fire-controls.md)). *(requires live testing — #736)*
...
![Ark Module vs Surface Population](../images/diagrams/5-colonies/5.2-ark-vs-surface.png)
...
![Population Growth Rate vs Colony Capacity](../images/charts/5-colonies/5.2-growth-rate-vs-capacity.png)
...
![Workforce Allocation Pipeline](../images/diagrams/5-colonies/5.2-workforce-allocation.svg)
...
![Workforce Allocation by Colony Cost](../images/charts/5-colonies/5.2-workforce-allocation.png)
...
This allocation reduces the effective manufacturing and mining workforce. On a [CC](../appendices/B-glossary.md#cc) 4.0 world, one quarter of your available workers are occupied maintaining agriculture and environmental systems rather than operating factories or mines. Combined with the 60% base workforce availability, a CC 4.0 colony with 10 million population has only 4.5 million effective industrial workers (10M x 60% x 75%).
...
| 25 million | Trade goods production becomes viable for sustained civilian routes *(unverified — #1259)* |
| 100 million | Manufacturing workforce stabilizes; governor bonuses become significant *(unverified — #1259)* |
| 300 million | Large-scale industrial colony; growth rate may exceed colonist shipment rates *(unverified — #1259)* |
...
> **Note:** The "approximately 60% available as workers" figure *(unverified -- #1257)* in [Section 5.2.4](#524-workforce-allocation) and the "30% manufacturing sector" figure here describe different partitioning models of the same population. The 60% figure accounts for non-working population (children, elderly, etc.), while the 70% service sector cap \hyperlink{ref-5.2-15}{[15]} further subdivides the working population between service and manufacturing roles. These two models are not reconciled: if 60% of population are workers and 70% of those are in the service sector, only 18% of total population would be in manufacturing -- yet the 30% manufacturing sector figure \hyperlink{ref-5.2-15}{[15]} implies a different calculation. The effective manufacturing workforce depends on which model the game applies in a given context, and the exact interaction between these figures has not been confirmed in the database *(unverified -- #1257)*.
...
- **Partially Staffed:** If the colony's manufacturing population is insufficient to staff all installations, each installation operates at reduced efficiency proportional to available workforce. *(unverified — #1211 — community sources indicate the efficiency penalty is applied uniformly to all production rather than by installation priority; the v1.12.0 priority allocation claim could not be verified against the changelog)*
...
- Reduced political stability *(unverified — #1211 — no database field links unemployment to stability; may not exist as a mechanic)*
- Slower population growth *(unverified — #1211 — no database field links unemployment to growth; may not exist as a mechanic)*
...
When a colony doesn't have enough population to staff all installations, Aurora prioritizes allocation. Generally: *(unverified — #1211 — community sources indicate there is no priority list; the game calculates total worker shortage and applies a uniform manufacturing efficiency penalty to all production equally)*
...
In C# Aurora, wealth generation is tied to industrial output rather than total population. Each 1 million workers in [TN](../appendices/B-glossary.md#tn) installations produces a baseline of 100 wealth per annum, regardless of whether their assigned installation is actively building or producing.
...
| **Build Cost** | 40 [BP](../appendices/B-glossary.md#bp) each \hyperlink{ref-5.2-6}{[6]} |
...
(Square Root of Size x Units x Morale) / 10,000 *(community-derived formula — requires live testing — #736)*
...
4. **Unrest Reduction Rate** = 100 x Police Modifier (points per year) *(requires live testing — #736)*
...
- **Low unrest (1-10):** Negligible impact on colony operations *(unverified — #1228)*
- **Moderate unrest (10-39):** Minor morale penalties but no production impact *(unverified — #1228)*
- **High unrest (40+):** Production penalties begin, with output numbers visibly declining *(requires live testing — #736)*
- **Severe unrest:** Continued increases can spiral into significant production loss *(unverified — #1228)*
...
- Increases as colony population grows and as the empire expands *(unverified — #1228)*
...
- Contribution is based on the number and strength of weapons on the ship *(unverified — #1228)*
- Example: A frigate with 4 lasers might contribute approximately 8 points of protection *(unverified — #1228)*
- Multiple armed ships in-system combine their contributions *(unverified — #1228)*

**Consequences of Insufficient Protection:**

- When Protection Actual is below Protection Required, the population's resistance modifier decreases *(unverified — #1228)*
- The goal is to keep all modifiers at 100%
- The longer protection remains below requirements, the worse the impact becomes *(unverified — #1228)*
- Eventually drives up unrest, which then causes production penalties *(unverified — #1228)*
- Manifests as "Unrest is increasing" alerts in the event log *(unverified — #1228)*
...
## 5.2.10 Logistics Eligibility Display (v2.8.0)

*Updated: v2026.02.02*

The Population window includes a checkbox option to display logistics eligibility codes alongside each population entry. When enabled, these codes indicate which logistics operations the colony can perform, helping players quickly identify supply chain capabilities across their empire. \hyperlink{ref-5.2-9}{[9]} \hyperlink{ref-5.2-16}{[16]}

### 5.2.10.1 Population Logistics Codes

*Added: v2026.02.02*

| Code | Meaning |
|------|---------|
| R | Refuelling Capability |
| O | Ordnance Transfer Capability |
| C | Cargo Handling Capability |
| M | Eligible for Orbital Mining |
...
\hypertarget{ref-5.2-16}{[16]}. Aurora Forums -- Steve Walmsley post, January 31, 2026 -- population logistics codes (R/O/C/M) for v2.8.0.
