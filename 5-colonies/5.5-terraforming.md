...
7. To reassign installations to a different gas, change the dropdown selection and quantity, then click Assign again; installations can be reassigned at any time without delay *(unverified — requires live testing of reassignment delay mechanics)*
8. Installations can be split across multiple gases simultaneously by assigning different quantities to different gas operations in separate assignments *(unverified — requires live testing of multi-gas assignment)*
...
> **Note:** Ship-based terraforming modules are assigned their target gas through fleet orders rather than the colony Environment tab. The ship must be in orbit around the target body and given a terraforming order specifying which gas to add or remove. *(unverified — requires live testing of ship-based terraforming fleet orders)*
...
- **Albedo** from surface water provides a minor temperature effect: 0.0015 x Hydro Extent %, to a maximum increase of 0.15 *(unverified — game engine constant, not stored in database)*
...
- Water vapor condenses at a rate of 0.1 atm per year, increasing Hydro Extent *(unverified — game engine constant, not stored in database)*
- Equilibrium water vapor pressure: Atmospheric Pressure x (Hydro Extent / 100) x 0.01 atm *(unverified — game engine constant, not stored in database)*
...
- Water vapor condenses at a rate of 0.1 atm per year, increasing Hydro Extent (surface water coverage) *(unverified — game engine constant, not stored in database)*
- Equilibrium water vapor pressure: Atmospheric Pressure x (Hydro Extent / 100) x 0.01 atm *(unverified — game engine constant, not stored in database)*
...
- Each warhead increases the dust level by an amount equal to its warhead size (strength) *(unverified — game engine mechanic, not stored in database)*
- Dust Level / 20,000 is added to the planet's Anti-Greenhouse Factor *(unverified — community-derived formula, requires live testing)*
- 20,000 dust = 1.0 added to the Anti-Greenhouse Factor (significant cooling) *(unverified — derived from dust formula above)*
