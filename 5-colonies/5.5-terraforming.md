...
7. To reassign installations to a different gas, change the dropdown selection and quantity, then click Assign again; installations can be reassigned at any time without delay *(unverified -- reassignment mechanics)*
8. Installations can be split across multiple gases simultaneously by assigning different quantities to different gas operations in separate assignments *(unverified -- multi-gas assignment)*
...
> **Note:** Ship-based terraforming modules are assigned their target gas through fleet orders rather than the colony Environment tab. The ship must be in orbit around the target body and given a terraforming order specifying which gas to add or remove. *(unverified -- ship-based terraforming assignment mechanic)*
