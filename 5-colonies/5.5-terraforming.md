...
7. To reassign installations to a different gas, change the dropdown selection and quantity, then click Assign again; installations can be reassigned at any time without delay *(unverified — #748 -- reassignment mechanics)*
8. Installations can be split across multiple gases simultaneously by assigning different quantities to different gas operations in separate assignments *(unverified — #748 -- multi-gas assignment)*
...
> **Note:** Ship-based terraforming modules are assigned their target gas through fleet orders rather than the colony Environment tab. The ship must be in orbit around the target body and given a terraforming order specifying which gas to add or remove. *(unverified — #748 -- ship-based terraforming assignment mechanic)*
...
- Cost: 500 Build Points per module *(unverified — #748 -- space-based module is a ship component, not in installation table)*
...
- Smaller worlds terraform significantly faster using the modifier: Earth Surface Area / Planet Surface Area *(unverified — #748 -- size modifier formula not stored in database)*
- Mars terraforms approximately 3.5x faster than Earth *(unverified — #748 -- derived from surface area ratio)*
- Ganymede terraforms approximately 6x faster than Earth *(unverified — #748)*
- Luna terraforms approximately 14x faster than Earth *(unverified — #748)*
...
- **Albedo** from surface water provides a minor temperature effect: 0.0015 x Hydro Extent %, to a maximum increase of 0.15 *(unverified — #748 -- albedo formula not stored in database)*
...
- Water vapor condenses at a rate of 0.1 atm per year, increasing Hydro Extent *(unverified — #748 -- condensation rate not stored in database)*
- Equilibrium water vapor pressure: Atmospheric Pressure x (Hydro Extent / 100) x 0.01 atm *(unverified — #748 -- equilibrium formula not stored in database)*
...
- Water vapor condenses at a rate of 0.1 atm per year, increasing Hydro Extent (surface water coverage) *(unverified — #748)*
- Equilibrium water vapor pressure: Atmospheric Pressure x (Hydro Extent / 100) x 0.01 atm *(unverified — #748)*
...
- Each warhead increases the dust level by an amount equal to its warhead size (strength) *(unverified — #748 -- bombardment mechanic not stored in database)*
- Dust Level / 20,000 is added to the planet's Anti-Greenhouse Factor *(unverified — #748 -- dust formula derived from community observation)*
- 20,000 dust = 1.0 added to the Anti-Greenhouse Factor (significant cooling) *(unverified — #748)*
