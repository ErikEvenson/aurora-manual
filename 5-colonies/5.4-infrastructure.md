...
- Capacity: 5,000 tons of minerals per year *(unverified -- mass driver capacity not stored in installation table; the MassDriverValue=1.0 is a multiplier)*
...
- May cost 4 Build Points per unit (twice the cost of standard infrastructure), but this has not been confirmed in the database where only standard Infrastructure (2 BP, ID 9) appears with LGInfrastructureValue=0.0. LG-Infrastructure is not listed as a separate installation; the doubled cost may be enforced in game logic not visible in the DB, or the same 2 BP installation may simply require more units on LG worlds. *(unverified -- LG-Infrastructure cost not confirmed in database)* \hyperlink{ref-5.4-14}{[14]}
...
Space Stations in C# Aurora are a distinct class of orbital structure defined by their use of **Structural Shell** instead of standard armour. Key characteristics: *(unverified -- structural shell stats not in installation database)*
