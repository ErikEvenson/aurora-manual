...
![](../images/art/section-art/11.4.png)
...
![Cloaking Effect on Detection Range](../images/diagrams/11-sensors-and-detection/11.4-cloaking-effect.svg)

![Cloaking Detection Range Comparison](../images/diagrams/11-sensors-and-detection/11.4-cloaking-detection.png)
...
```
Cloak Size = Ship Size ([HS](../appendices/B-glossary.md#hs)) * Cloak Size Multiplier
```
...
- **Cloaked Cross-Section:** A ship with an operational cloak is always treated as having its cloaked (reduced) cross-section for both detection and combat targeting purposes. *(unverified — [#1284](https://github.com/ErikEvenson/aurora-manual/issues/1284) -- v2.0.0 changelog)*
- **Damaged Cloak Behavior (v2.0.0):** In v2.0.0+, the current state of the cloak is taken into account. If the cloak is damaged, the cross-section reverts to normal for both detection and combat purposes. This differs from v1.13.0 where a damaged cloak still provided the full cloaked cross-section. *(unverified — [#1284](https://github.com/ErikEvenson/aurora-manual/issues/1284) -- v2.0.0 changelog)*
- **Fire Control Lock-On:** In v2.0.0, missile fire controls properly use the reduced cloaked cross-section when targeting cloaked vessels. A previous bug allowed missile fire controls to lock on using the full (uncloaked) cross-section, which has been fixed. *(unverified — [#1284](https://github.com/ErikEvenson/aurora-manual/issues/1284) -- v2.0.0 bug fix)*
...
![EMCON State Lifecycle](../images/diagrams/11-sensors-and-detection/11.4-emcon-lifecycle.svg)
...
- Fire controls are not available (cannot guide missiles or engage with beam weapons) *(unverified — [#1283](https://github.com/ErikEvenson/aurora-manual/issues/1283))*

**EMCON Levels**

Aurora C# supports multiple EMCON settings that allow graduated emission control:

- **Full EMCON:** All active emissions suppressed. Ship is completely passive. *(unverified — [#1283](https://github.com/ErikEvenson/aurora-manual/issues/1283))*
- **EMCON with distance threshold:** Active sensors activate automatically when a target is detected within a specified range by passive sensors. *(unverified — [#1283](https://github.com/ErikEvenson/aurora-manual/issues/1283))*
...
- **In-flight missiles lose guidance:** If a ship activates EMCON while its missiles are in flight, the missile fire control can no longer provide course updates. Missiles without onboard sensors go ballistic (flying to the target's last known position). *(unverified — [#1283](https://github.com/ErikEvenson/aurora-manual/issues/1283))*
...
See [Section 12.1 Fire Controls](../12-combat/12.1-fire-controls.md) for [MFC](../appendices/B-glossary.md#mfc) guidance requirements and [Section 12.3 Missiles](../12-combat/12.3-missiles.md) for missile guidance modes.
...
![Jump Point Ambush Positioning](../images/diagrams/11-sensors-and-detection/11.4-ambush-positioning.svg)
