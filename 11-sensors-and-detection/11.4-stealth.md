...
- **Damaged Cloak Behavior (v2.0.0):** In v2.0.0+, the current state of the cloak is taken into account. If the cloak is damaged, the cross-section reverts to normal for both detection and combat purposes. This differs from v1.13.0 where a damaged cloak still provided the full cloaked cross-section.
- **Fire Control Lock-On:** In v2.0.0, missile fire controls properly use the reduced cloaked cross-section when targeting cloaked vessels. A previous bug allowed missile fire controls to lock on using the full (uncloaked) cross-section, which has been fixed.
