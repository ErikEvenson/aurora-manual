...
![](../images/art/section-art/14.2.png)
...
![Figure 14.2.1: Maintenance Clock vs. Component Failure Risk](../images/charts/14-logistics/14.2.1-maintenance-failure-curve.png)
...
- **Repair:** Failed components can be repaired using [Maintenance Supply Points (MSP)](../appendices/B-glossary.md#msp) if the ship has them available (see Maintenance Supply Points below)
...
**Overcrowding from Destroyed Crew Quarters**

When crew quarters are destroyed by maintenance failures (or combat), the ship becomes overcrowded. Overcrowding accelerates the rate at which deployment time accumulates, with observed multipliers of **4-5x** the normal rate *(unverified — [#1353](https://github.com/ErikEvenson/aurora-manual/issues/1353))*. This creates a dangerous feedback loop:

1. Extended over-deployment causes maintenance failures
2. Maintenance failures destroy components, including crew quarters
3. Destroyed crew quarters cause overcrowding
4. Overcrowding accelerates deployment time accumulation (4-5x rate)
5. Faster deployment accumulation causes more frequent maintenance failures
6. The cycle repeats, rapidly degrading the ship

This "death spiral" can progress from manageable damage to total ship loss within weeks of game time *(unverified — [#1353](https://github.com/ErikEvenson/aurora-manual/issues/1353))*.

**Catastrophic Ship Destruction**

Ships at extreme over-deployment (observed at 1000%+ deployment) with multiple destroyed components can suffer **catastrophic failure and explode**, destroying the ship entirely *(unverified — [#1353](https://github.com/ErikEvenson/aurora-manual/issues/1353))*. Surviving crew are placed in life pods. The exact threshold and probability for catastrophic destruction are unknown, but the risk increases with the severity and number of unrepaired component failures.

**Life Pod Endurance**

Crew who survive a ship's destruction are placed in life pods with limited endurance. Observed life pod endurance is approximately **15 days**, after which all crew in the pods perish *(unverified — [#1353](https://github.com/ErikEvenson/aurora-manual/issues/1353))*. Rescue operations must be mounted quickly — any task group can pick up life pods using the "Rescue Survivors" order or the "Pickup Nearest Lifepod" standing order (see [Section 9.5.4 Standing Orders](../9-fleet-management/9.5-orders.md#954-standing-orders)).

> **Warning:** Once the overcrowding death spiral begins, the ship may be beyond saving. If crew quarters are destroyed and the ship is far from a shipyard, consider whether a rescue mission can reach the ship before cascading failures destroy it. Detaching an immobilized ship (dead engine) from a task group prevents it from limiting the group's speed to 1 km/s.
...
- If a ship's crew quarters are destroyed, prioritize immediate return to port — the overcrowding death spiral can destroy the ship within weeks

### 14.2.1.2 Deployment Clock

![Maintenance Clock vs Deployment Clock Comparison](../images/diagrams/14-logistics/14.2-maintenance-deployment-clocks.png)
...
- **Deployment Time Setting:** Each ship class has an "Intended Deployment Time" (measured in months) set in the Class Design window (see [Section 8.6 Other Components -- Deployment Time](../8-ship-design/8.6-other-components.md)). The default is 3 months for new designs \hyperlink{ref-14.2-6}{[6]}
...
![Figure 14.2.2: Maintenance Facility Capacity Scaling by Tech Level](../images/charts/14-logistics/14.2.2-maintenance-capacity-scaling.png)
...
- **Cost**: 60 [BP](../appendices/B-glossary.md#bp) (Build Points) per facility \hyperlink{ref-14.2-1}{[1]}
...
Multiple populations and maintenance ships of the same faction at one location combine their capacities into a Total Maintenance Capacity. This enables deep space bases with only maintenance modules and no planetary infrastructure *(unverified — [#849](https://github.com/ErikEvenson/aurora-manual/issues/849), requires live testing)*.
...
- **Normal Maintenance**: Ships require MSP equal to Class Cost / 4, adjusted by EMR percentage *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837), requires live testing -- hardcoded game logic)*
- **Overhaul**: Ships require MSP equal to Class Cost, adjusted by EMR percentage *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837), requires live testing -- hardcoded game logic)*
...
MSP production consumes three minerals *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837), requires live testing -- hardcoded game logic, not present in database)*:
...
- **Combined with Fuel Warnings:** This feature works alongside the fuel warning system (see [Section 14.1 Fuel](14.1-fuel.md)), allowing comprehensive logistics monitoring for each population
...
> **Warning:** The "LG: Overhaul at Colony" standing order resets the deployment clock and restores morale (shore leave), but does **not** resupply [MSP](../appendices/B-glossary.md#msp) from the colony stockpile. Ships completing a standalone overhaul via this standing order will depart with their current MSP level — potentially 0. To ensure MSP are loaded, use the "Refuel, Resupply, and Overhaul" conditional order ([Section 14.2.5.2](#1425-the-replenish-standing-order)), or add a separate "Refuel and Resupply from Colony" movement order before or after the overhaul \hyperlink{ref-14.2-8}{[8]}.

![Figure 14.2a: MSP Resupply -- Refuel and Resupply from Colony](../images/screenshots/14-logistics/14.2-msp-resupply.png)
...
> **Tip:** Prefer "Refuel, Resupply, and Overhaul" over the standalone "LG: Overhaul at Colony" standing order for routine maintenance cycles. The standalone overhaul resets deployment and morale but does **not** load MSP, fuel, or ordnance. Only the combined order handles all resupply alongside the overhaul \hyperlink{ref-14.2-8}{[8]}.
...
\hypertarget{ref-14.2-8}{[8]}. In-game testing (Game 2 playthrough, Discussion #1350) -- Confirmed: "LG: Overhaul at Colony" standing order completed overhaul (deployment reset, morale restored) but MSP remained at 0/75 despite colony having 147,000+ MSP. A separate "Refuel and Resupply from Colony" order was required to load MSP.
