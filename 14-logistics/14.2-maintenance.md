...
- **Deployment Time Setting:** Each ship class has an "Intended Deployment Time" (measured in months) set in the Class Design window (see [Section 8.6 Other Components -- Deployment Time](../8-ship-design/8.6-other-components.md)). The default is 3 months for new designs *(unverified — #751)*
...
- **Normal Maintenance**: Ships require MSP equal to Class Cost / 4, adjusted by EMR percentage *(unverified — #751)*
- **Overhaul**: Ships require MSP equal to Class Cost, adjusted by EMR percentage *(unverified — #751)*
...
MSP production consumes three minerals *(unverified — #751 -- not present in game database; values are hardcoded in game logic)*:
...
Ships that abandon overhaul suffer severe performance penalties that gradually improve over 30 days. The ship receives an "Overhaul Factor" starting at 0.01 that increases linearly to 1.0 over thirty days *(unverified — #751)*.
