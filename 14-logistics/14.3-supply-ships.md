...
| Cargo Type | Formula |
|---|---|
| Cargo | (Cargo Capacity x 20) / Cargo Handling Modifier *(unverified — #755)* |
| Colonists | (Colonist Capacity x 10) / Cargo Handling Modifier *(unverified — #755)* |
| Ground Forces | (Size of Force Loaded x 20) / Cargo Handling Modifier *(unverified — #755)* |

### 14.3.4.3 Facility Bonuses

If there are one or more Spaceports or Cargo Handling Stations on the planet, that adds a single extra cargo shuttle bay to the calculation *(unverified — #755)*. Multiple facilities do not stack beyond this single bonus.
...
- All VB6 payment rates are halved as a baseline but multiplied by the number of systems traveled *(unverified — #755)*
- A ship traveling two systems receives the equivalent of the old VB6 payment rate *(unverified — #755)*
- A ship traveling five systems receives 250% of the original VB6 rate *(unverified — #755)*
...
- Payment = (Base Rate / 2) * Number of Systems Traveled *(unverified — #755)*
- Tax = Base Rate * Number of Systems Traveled *(unverified — #755)*
...
- **Recreational Locations**: Ships with recreational modules, or populations of 50,000 or more *(unverified — #755)*
- At recreational sites, deployment clocks reduce at 10x the passage rate *(unverified — #755)*
- **Parasites in Hangars**: Clock reduction = Time Passed x 10 x (1 - Mothership Deployment Modifier) *(unverified — #755)*
- **Mothership Modifier** = Mothership Deployment Clock / Mothership Class Intended Deployment Time *(unverified — #755)*

**Morale Calculations:**

- Baseline morale is 100% unless deployment clock exceeds intended deployment time
- When exceeded: Morale = Intended Deployment Time / Deployment Clock *(unverified — #755)*
- Reduced crews multiply morale by: (Current Crew / Class Crew) x 2 *(unverified — #755)*
- Morale floor: 25% minimum *(unverified — #755)*
...
> Overcrowding Modifier = (Required Accommodation / Actual Accommodation)^2 *(unverified — #755)*

**Effects:**

- Deployment clock increases at Overcrowding Modifier x normal rate *(unverified — #755)*
- If the modifier exceeds 1.5, life support risks damage *(unverified — #755)*
- Failure chance per construction phase = Overcrowding Modifier x 100 x (Increment Length / Year Length) *(unverified — #755)*
...
- Deployment clock increases at 12x normal rate *(unverified — #755)*
- Morale drops immediately to 10% *(unverified — #755)*
- The crew takes casualties from 4% to 80% (4d20 roll) of the remaining crew each construction phase *(unverified — #755)*
- Commanders have a death chance equal to half the crew casualty percentage *(unverified — #755)*
...
Maintenance supplies are transferred at the rate of 10 per hour, multiplied by the number of cargo shuttle bays and the racial shuttle technology *(unverified — #755)*. Multiple ships can resupply simultaneously from spaceports, stations, and facilities, though they must remain stationary during the process.
