...
![](../images/art/section-art/9.1.png)

![Figure 9.1: Shipyard Management](../images/screenshots/9-fleet-management/9.1-shipyard-management.png)
...
**Naval Shipyards** build warships -- any vessel designed with military-grade components such as weapons, fire control systems, military-grade engines, or armor beyond the minimum. A Naval Shipyard Complex costs 2,400 [BP](../appendices/B-glossary.md#bp) and requires 1,200 Duranium + 1,200 Neutronium \hyperlink{ref-9.1-5}{[5]}. Naval shipyards can build military vessels of any size up to the yard's current capacity. They start with a base capacity of 1,000 tons \hyperlink{ref-9.1-1}{[1]} and are more expensive to expand. All shipyard types share the same base build rate: 400 BP/year per slipway \hyperlink{ref-9.1-2}{[2]} (upgradeable via [Section 7.4.2 Construction and Production](../7-research/7.4-tech-categories.md) research).

**Commercial Shipyards** build civilian and support vessels -- freighters, colony ships, fuel harvesters, terraformers, salvagers, and any other ship class that does not incorporate restricted military components. A Commercial Shipyard Complex has the same cost as naval: 2,400 BP requiring 1,200 Duranium + 1,200 Neutronium \hyperlink{ref-9.1-5}{[5]}. Commercial shipyards provide 10x the capacity of a naval shipyard for the same cost and workforce -- starting at 10,000 tons by default \hyperlink{ref-9.1-4}{[4]}. They are significantly cheaper to expand per ton of capacity added. However, commercial shipyards cannot build military vessels.

> **v2.8.0+:** Light Naval Shipyards are a new shipyard type introduced in version 2.8.0 as part of the Fighter/FAC unification changes.

**Light Naval Shipyards** are a specialized fourth shipyard type designated "L" for building small warships of 1,000 tons or less. Key characteristics:

- Identical cost to Naval Shipyards
- Fixed capacity of 1,000 tons per slipway (capacity cannot be upgraded, even via SM Mode)
- Only "Add Slipway" and "SM Modification" upgrades are available — no capacity expansion tasks
- No retooling required — can build any military ship class of 1,000 tons or less without retooling
- Flexible refitting — can refit between any two classes at or below 1,000 tons with no restrictions (size-difference cost still applies)
- Pairs with the renamed **Light Naval Factories** (formerly Fighter Factories) which can build any ship of 1,000 tons or less

**Contrast with Standard Naval Shipyards:** Standard naval shipyards require retooling when switching between ship classes, but can expand their capacity indefinitely to build larger vessels. Light Naval Shipyards trade expansion flexibility for production flexibility — the fixed 1,000-ton cap means they're specialized for small warships, but the elimination of retooling requirements makes them ideal for producing diverse fleets of fighters, FACs, and corvettes without shipyard management overhead
...
### 9.1.3.1 Repeat Shipyard Tasks

*Added: v2026.02.03*

> **v2.8.0+:** The repeat task feature was introduced in Aurora C# v2.8.0. Earlier versions require manual task creation for each ship.
...
When you enter a repeat count (e.g., "5") and create a task, the shipyard establishes a task counter that decrements with each completion:

1. Set repeat count to 5, click Create Task
2. First ship completes -- counter decrements to 4, next ship auto-queues
3. Second ship completes -- counter decrements to 3, next ship auto-queues
4. Process continues until counter reaches 0

The Shipyard Tasks display shows the remaining task count (e.g., "3 remaining") alongside the current ship's completion date and progress.

**Auto-Queue Behavior:**

When a task completes and the counter is greater than zero, the shipyard automatically queues the next identical task without player intervention:

- **Construction:** Automatically lays down the next ship using the class naming theme for sequential ship names
- **Refit:** Automatically begins refitting the next ship of the original class to the target class
- **Scrap:** Automatically scraps the next ship of the same class
- **Repair:** Automatically repairs the next damaged ship of the same class

**Task Abandonment:**

The repeat sequence terminates early (abandoning remaining count) if:

- No suitable ship is available for the operation (refit/scrap/repair with no eligible ships remaining)
- Required alien components are lacking for construction
- The shipyard is retooled to a different class
- Minerals required for the next task are unavailable (construction pauses but does not abandon)

**Completion Date Calculation:**

The displayed Completion Date reflects the total time for all remaining ships in the repeat sequence, not just the current ship. As each ship completes and the counter decrements, the Completion Date updates to reflect the reduced workload.
...
\hypertarget{ref-9.1-5}{[5]} AuroraDB.db, DIM_PlanetaryInstallation table: Naval Shipyard Complex (PlanetaryInstallationID=4) Cost=2400 BP, Duranium=1200, Neutronium=1200. Commercial Shipyard Complex (PlanetaryInstallationID=32) Cost=2400 BP, Duranium=1200, Neutronium=1200. Both shipyard types have identical construction costs.
