...
### Construction Task Workflow (Step-by-Step)
...
**Intelligent Scrapping:** In C# Aurora, the scrapping/deleting process is much more intelligent about handling ship contents *(unverified)*. When a ship is scrapped:
...
**Mineral Recovery:** Scrapping a ship returns a percentage of the minerals used in its construction back to the colony's stockpile. The recovery rate is typically around 25-50% of the original mineral cost *(unverified)*, depending on game settings. This makes scrapping worthwhile for large obsolete vessels even if the recovered minerals are modest compared to active mining.
