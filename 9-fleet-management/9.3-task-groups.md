...
**Sub-Pulse Order Processing:** Aurora C# processes orders within each time increment. Conditional orders are evaluated at each sub-pulse *(unverified)*, meaning a task group can detect a threat and respond within the same time increment.
...
```
*(unverified formula -- the Fleet Training bonus (BonusID 37) exists as a distinct bonus type from Crew Training (BonusID 1); the formula may use Fleet Training rather than Crew Training)*
...
\hypertarget{ref-9.3-1}{[1]} AuroraDB.db, DIM_CommanderBonusType table: Naval commander bonus types include Reaction (BonusID=13), Crew Training (BonusID=1), Logistics (BonusID=24), Carrier Operations (BonusID=7), Tactical (BonusID=21), Survey (BonusID=2), Engineering (BonusID=28), and others. All are percentage-based bonuses.
...
\hypertarget{ref-9.3-3}{[3]} AuroraDB.db, FCT_TechSystem table: Fuel Consumption technology entries measured in "Litres per Engine Power Hour" units, confirming time-based fuel consumption model.

\hypertarget{ref-9.3-4}{[4]} AuroraDB.db, DIM_CommanderBonusType table: Fleet Training (BonusID=37) exists as a distinct naval bonus type separate from Crew Training (BonusID=1). Both are percentage-based (Percentage=1).
