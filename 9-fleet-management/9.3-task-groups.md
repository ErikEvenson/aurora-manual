...
- **Initiative** -- Determines the task group's movement order during combat (see below) *(unverified)*
- **Navigation** -- Reduces transit time and improves pathfinding *(unverified)*
- **Training** -- Improves crew performance over time *(unverified)*
- **Logistics** -- Affects efficiency of loading/unloading operations *(unverified)*

Officers are assigned from your officer pool through the Naval Organization window. A task group without a commander operates at reduced effectiveness *(unverified)*.
...
**Speed and Fuel Consumption:** Ships in a task group all travel at the group's speed, not their individual maximum speeds. Ships capable of higher speeds that are throttled down to match the group consume less fuel per distance traveled than at their maximum speed. Fuel consumption is proportional to speed *(unverified)*.
...
**Sub-Pulse Order Processing:** Aurora C# processes orders within each time increment. Conditional orders are evaluated at each sub-pulse, meaning a task group can detect a threat and respond within the same 5-second increment (the smallest time unit) *(unverified)*.
...
- **Doubled Maintenance Failures:** Maintenance failure rate is doubled compared to normal operations *(unverified)*
...
- **Fuel Consumption:** Fuel is consumed as if engines are running at 10% power continuously, simulating training exercises *(unverified)*
