...
**Sub-Pulse Order Processing:** Aurora C# processes orders within each time increment. Conditional orders are evaluated at each sub-pulse *(unverified — #750 -- requires live testing)*, meaning a task group can detect a threat and respond within the same time increment.
...
```
*(unverified formula — #750 -- requires live testing; the Fleet Training bonus (BonusID 37) exists as a distinct bonus type from Crew Training (BonusID 1) \hyperlink{ref-9.3-4}{[4]}; the formula may use Fleet Training rather than Crew Training)*
...
- **Doubled Maintenance Failures:** Maintenance failure rate is doubled compared to normal operations *(unverified — #750 -- requires live testing)*
...
- **Fuel Consumption:** Fuel is consumed as if engines are running at 10% power continuously, simulating training exercises *(unverified — #750 -- requires live testing)*
