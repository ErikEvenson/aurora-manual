...
**Sub-Pulse Order Processing:** Aurora C# processes orders within each time increment. Conditional orders are evaluated at each sub-pulse *(unverified — #750)*, meaning a task group can detect a threat and respond within the same time increment.
...
- **Doubled Maintenance Failures:** Maintenance failure rate is doubled compared to normal operations *(unverified — #750)*
...
- **Fuel Consumption:** Fuel is consumed as if engines are running at 10% power continuously, simulating training exercises *(unverified — #750)*
