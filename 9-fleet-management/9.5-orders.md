...
**Load Ordnance:** Replenishes missile magazines and ammunition from the colony's ordnance stockpile (see [Section 14.1 Supply and Maintenance](../14-logistics/14.1-supply-and-maintenance.md)). Critical for missile-heavy fleets that expend ammunition in combat.
...
- **Non-Combat Resupply** -- Ground forces resupply out of combat if they are at a population and logistics units are available. The formation commander's logistics bonus reduces the total supply amount needed; unlike combat resupply where the bonus provides a chance for "free" supply per combat round, non-combat resupply treats this bonus as a deterministic cost reduction.
...
| Pickup Nearest Lifepod | S | Automatically collects lifepods within the same system. Multiple fleets coordinate their pickups so that two fleets do not attempt to collect the same lifepod. As of v1.12.0, a "Rescue Only" flag prevents lifepod pickup from triggering during unrelated transit orders; the fleet will only divert for lifepods when the standing order is the active order. |
...
| Checkbox | Effect |
|----------|--------|
| Assume Fleet is Jump Capable | Algorithm assumes the fleet can transit any jump point without checking actual jump capability. As of v2.0, this setting is applied per-fleet rather than globally, allowing different fleets to have different jump assumptions. |
...
### Fleet Raise/Lower Shields Button
...
### Fleet Distance and Time Display
...
For time increments of one hour or longer, Aurora monitors ships moving toward hostile contacts or waypoints containing hostile vessels. When a moving ship would reach within 500,000 km of its destination, the increment automatically reduces to the time needed to reach that distance, rounded down to the nearest valid sub-pulse length.
...
- [9.3 Task Groups](9.3-task-groups.md) -- Task group formation and conditional order basics
- [9.4 Fleet Organization](9.4-fleet-organization.md) -- Fleet hierarchy and admin commands
- [10.3 Jump Points](../10-navigation/10.3-jump-points.md) -- Jump point transit mechanics referenced by movement orders
- [10.4 Waypoints](../10-navigation/10.4-waypoints.md) -- Waypoints used as movement destinations and patrol points
- [14.1 Supply and Maintenance](../14-logistics/14.1-supply-and-maintenance.md) -- Logistics operations for resupply orders
- [12.1 Combat Mechanics](../12-combat/12.1-combat-mechanics.md) -- Combat engagement mechanics for combat orders
