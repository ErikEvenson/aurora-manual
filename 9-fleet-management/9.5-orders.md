...
> **v2.8.0+:** Fleet orders are now evaluated from newest to oldest. When multiple fleets are eligible for the same task (trade contracts, standing orders, conditional triggers), the most recently created fleet is checked first. This affects which fleet is assigned to a given contract and how standing orders are distributed across your navy. Players managing multiple fleets with similar standing orders should be aware that newer fleets will claim available tasks before older ones. To prioritize a specific fleet, either create it after other fleets or delete and recreate its standing orders to make them "newer" in the evaluation queue.
...
**Load Ordnance:** Replenishes missile magazines and ammunition from the colony's ordnance stockpile *(unverified — #1232)* (see [Section 6.3 Construction](../6-economy-and-industry/6.3-construction.md) for ordnance factory production and [Section 12.3 Missiles](../12-combat/12.3-missiles.md) for ordnance transfer mechanics). Critical for missile-heavy fleets that expend ammunition in combat.
...
> **v2.8.0+:** This order loads as many ship components as possible from the destination population, prioritized in descending order of component size.

Useful for logistics runs collecting prefabricated components for deployment elsewhere. Larger components are loaded first to maximize the use of available cargo capacity.
...
| Facility | [BP](../appendices/B-glossary.md#bp) Cost | Size | Notes |
|-----------|---------|------|-------|
...
| Civilian Contract Fulfillment | A | Fulfil civilian shipping contracts (v2.8.0+) |
...
> **v2.8.0+:** This standing order type enables fleets to execute civilian shipping contracts autonomously.

- **Contract Execution:** Fleets with this standing order automatically accept and fulfil civilian shipping contracts without requiring player-initiated payment
- **Fuel Consumption:** The fleet consumes fuel during contract execution as normal (unlike civilian shipping which operates independently)
- **Refuelling:** Fleets can refuel at destinations marked with the "Destination for Automated Refuel" checkbox during contract execution
- **Use Case:** Useful for supplementing civilian shipping capacity, ensuring critical trade routes are serviced when civilian shipping is insufficient, or keeping military cargo assets productively engaged during peacetime

> **Tip:** Combine Civilian Contract Fulfillment with the "Load All Ship Components" logistics order (see [Section 9.5.3](#953-logistics-orders)) to create versatile cargo fleets that can both service civilian contracts and collect prefabricated components during downtime between contract assignments.
...
- **Refuel/Resupply** addresses consumables (fuel and [MSP](../appendices/B-glossary.md#msp)) but does NOT reset the deployment timer
...
- **"LG: Overhaul at Colony"** (standing order) resets deployment and morale but does **not** resupply MSP, fuel, or ordnance. Use "Refuel, Resupply, and Overhaul" (conditional order) for full servicing. See [Section 14.2.4 Overhaul](../14-logistics/14.2-maintenance.md#1424-overhaul) for details
...
| Checkbox | Effect |
|----------|--------|
| Assume Fleet is Jump Capable | Assumes any [JP](../appendices/B-glossary.md#jp) is transitable (per-fleet as of v2.0.0) |
...
### 9.5.7.1 Lagrange Point Routing Options

*Added: v2026.02.03*

> **v2.8.0+:** Dynamic Lagrange Point Orders provide three routing modes that control how fleets interact with Lagrange Points during movement order execution \hyperlink{ref-9.5-13}{[13]}.

Lagrange Points allow instant intra-system travel between gravitationally stable positions (see [Section 10.1.5 Orbits](../10-navigation/10.1-movement-mechanics.md#1015-orbits) for Lagrange Point mechanics). The routing mode determines when and how the game evaluates LP shortcuts for your fleet orders.

**Three Routing Modes:**

| Mode | Evaluation Timing | Best For |
|------|-------------------|----------|
| **Manual LP** | Never (player selects LPs explicitly) | Tactical control, avoiding threats near LPs |
| **Plotted LP** | At order creation time | Predictable routing, coordinated movements |
| **Dynamic LP** | During order execution | Long-range operations, patrol routes |

**Manual LP:** Fleets ignore Lagrange Points entirely unless you explicitly add an LP as a waypoint in the order queue. This mode maintains full player control over routing decisions. Use when:

- You need precise control over fleet paths
- Known threats exist near Lagrange Point locations
- Coordinating multiple fleets through specific routes

**Plotted LP:** The game evaluates LP shortcuts when each movement order is created. If using an LP shortens the trip at that moment, the route is plotted through the LP. The plotted route remains fixed even if orbital positions change during transit. Use when:

- You want optimized routing at order time
- Fleet movements need to be predictable for coordination
- Orders are created close to execution time (short lead time)

**Dynamic LP:** The game re-evaluates LP routing when orders are assigned to fleets without existing orders, and when orders complete mid-pulse. This allows fleets to take advantage of LP shortcuts that become available during long transits as orbital positions shift. Use when:

- Fleets execute extended operations (multi-week or longer)
- Patrol routes may benefit from changing orbital geometry
- Exploration fleets operate autonomously in unfamiliar systems

**Limitation:** A fleet may begin executing a second order before that order receives its dynamic LP evaluation. In practice, this minor timing gap rarely affects operations, and dynamic routing still provides superior optimization compared to static pre-planning for long-duration orders.

**Setting the Mode:** The LP routing mode is set per-fleet in the Naval Organization window's Movement Orders tab via three radio buttons.
...
\hypertarget{ref-9.5-13}{[13]} Aurora Forums, "v2.8.0 Changes List" (aurora2.pentarch.org/index.php?topic=13884.0) -- Dynamic Lagrange Point Orders: three routing modes (Manual, Plotted, Dynamic) for fleet LP routing control
