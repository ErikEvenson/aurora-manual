...
**Cargo Size:** Each infrastructure unit occupies 2,500 tons of cargo capacity (displayed as "0.1" in the Load Installation interface, meaning 0.1 of a 25,000-ton cargo hold). A freighter with 25,000 tons of cargo capacity carries 10 infrastructure per trip. Two freighters in the same fleet carry 20 infrastructure per round trip.

**Capacity Rule of Thumb:** Each infrastructure unit supports approximately 5,000 population on a colony cost 1.0 body. Twenty infrastructure supports roughly 100,000 population.
...
|-----------|---------|------|-------|
| Spaceport | 2,400 | 4x research fac. | Unlimited ships; stationary; freighter-transportable |
| Refuelling Station | 1,200 | 10 factories | No workers; freighter-transportable |
| Refuelling Hub | 2,400 | 100kt (component) | Ship-mounted; for deep-space stations |
| Refuelling System | 10-100 | 500t (component) | Single-ship refuelling from tanker |
...
Each ship class has a designed "Intended Deployment Time" (also called Max Deployment Time) set during the ship design phase. This value represents how long the ship can operate before requiring overhaul. When the deployment timer exceeds this threshold:
...
The "Max Deployment Time" order setting works with the "Deployment Exceeded" conditional trigger to automate overhaul returns:
