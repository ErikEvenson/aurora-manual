...
**Command Radius:**
Admin Commands have a command radius that doubles with each level increase. Ships within this radius receive bonuses from the command's assigned commander. The radius progression is: level 1 = radius 1, level 2 = radius 2, level 4 = radius 3, level 8 = radius 4 *(unverified)*.

**Seven Command Types:**

| Type | Primary Bonuses | Notes |
|------|----------------|-------|
| Naval | 25% of commander bonus for Crew Training, Reaction, Engineering, Tactical *(unverified)* | Standard combat command |
| Patrol | Same as Naval *(unverified)* | Operates at twice the command radius *(unverified)* |
| Survey | Survey and Engineering bonuses *(unverified)* | Operates at twice the command radius *(unverified)* |
| Logistics | Cargo load/unload speed, reduced mining/terraforming *(unverified)* | Supply chain management |
| Industrial | Mining and Terraforming, secondary Logistics *(unverified)* | Resource extraction focus |
| General | 10% of commander bonus across multiple skills *(unverified)* | Versatile but diluted |
...
The required rank for an admin command is determined by the highest of four factors *(unverified)*:
