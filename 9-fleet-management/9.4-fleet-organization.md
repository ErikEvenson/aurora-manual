...
- Each Naval Headquarters level costs 1200 BP and can support multiple Admin Commands

**Command Radius:**
Admin Commands have a command radius that doubles with each level increase. Ships within this radius receive bonuses from the command's assigned commander. The radius progression is: level 1 = radius 1, level 2 = radius 2, level 4 = radius 3, level 8 = radius 4.

**Seven Command Types:**

| Type | Primary Bonuses | Notes |
|------|----------------|-------|
| Naval | 25% of commander bonus for Crew Training, Reaction, Engineering, Tactical | Standard combat command |
| Patrol | Same as Naval | Operates at twice the command radius |
| Survey | Survey and Engineering bonuses | Operates at twice the command radius |
| Logistics | Cargo load/unload speed, reduced mining/terraforming | Supply chain management |
| Industrial | Mining and Terraforming, secondary Logistics | Resource extraction focus |
| General | 10% of commander bonus across multiple skills | Versatile but diluted |
| Training | Enables fleet training using commander crew training ratings | Only functions within command radius |

**Hierarchical Bonuses:** Nested commands multiply bonuses -- a ship under a command within another command receives compounded benefits from the full chain.

**Admin Command Required Ranks (v2.0.0):**
The required rank for an admin command is determined by the highest of four factors:
...
### Worked Example: Early-Game Command Hierarchy
...
- Fleet Admirals can report to other Fleet Admirals -- same-rank reporting is permitted
- Promotion has no functional penalty -- a promoted officer operates at the assigned rank regardless of promotion points (low points may limit talent bonuses but do not prevent the rank from working)
- Logistics skill affects loading/unloading speed for cargo-related commands
- Mining skill benefits industrial commands (sorium harvesters, orbital miners)
- Reaction skill matters for defense/patrol commands
- Commercial ships do not gain fleet training or crew grade -- these are military-only mechanics
...
## 9.4.2 Assignment of Ships to Populations
...
- A tug vessel must be capable of tractoring
- The target fleet must be present in the system
- Standard tractor beam range and power limitations apply
...
## 9.4.4 Transfer Fleet to Alien Race
...
## 9.4.9 Fleet Deletion Protection
...
## 9.4.12 Missile Salvos Tab (v2.2.1)
...
## 9.4.13 Fleet Window UI Navigation (v1.13.0)
