...
**Hierarchical Bonuses:** Nested commands multiply bonuses -- a ship under a command within another command receives compounded benefits from the full chain. *(unverified -- bonus compounding mechanics are documented in community sources)*

**Admin Command Required Ranks (v2.0.0):**
The required rank for an admin command is determined by the highest of four factors *(unverified -- rank requirement formula is documented in community sources but not directly in the database)*:
...
- Fleet Admirals can report to other Fleet Admirals -- same-rank reporting is permitted *(unverified)*
- Promotion has no functional penalty -- a promoted officer operates at the assigned rank regardless of promotion points *(unverified)*
...
- Commercial ships do not gain fleet training or crew grade -- these are military-only mechanics *(unverified)*
