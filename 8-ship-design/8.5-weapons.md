...
![](../images/art/section-art/8.5.png)
...
![Weapons Damage Comparison: Laser vs Railgun](../images/charts/8-ship-design/8.5-weapons-damage-comparison.png)
...
- Damage does not decrease with range (full damage at any distance within maximum range) *(unverified — #1210 — community sources suggest railguns may have damage falloff similar to lasers)*
...
The flat damage profile (no fall-off) makes railguns attractive when you expect to fight at consistent medium ranges. A railgun that deals 10 damage does 10 damage whether the target is at 10,000 km or at maximum range. *(unverified — #1210 — community sources suggest railguns may have damage falloff similar to lasers; requires in-game testing)*
...
- When facing enemies that try to maintain distance (railgun damage does not diminish) *(unverified — #1210)*
...
- **Rate of fire (ROF):** Determined by technology. Measured in shots per 5-second fire control cycle. ROF ranges from 2 (starting tech) up to 8 at the highest tech level \hyperlink{ref-8.5-db-gauss-rof}{[9]}. Higher Gauss Cannon ROF tech = more shots per cycle.
...
A single gauss cannon mount fires multiple rounds per cycle. The number of rounds increases with technology, from 2 rounds at starting tech to 8 rounds at the highest tech level. Combined with multiple mounts sharing a fire control, this creates a wall of defensive fire.
...
Launcher size determines the maximum missile size (in [MSP](../appendices/B-glossary.md#msp) -- Missile Size Points) that can be fired. Launchers can now be designed in fractional sizes:
...
- **Ordnance Transfer Points:** Spaceports, Ordnance Transfer Stations, or Ordnance Transfer Hubs -- reloading at these facilities is 10x slower than hangar reloading *(unverified — #1230)*
...
- **[ECM](../appendices/B-glossary.md#ecm):** Electronic countermeasures to avoid point defense
...
![Meson Armor Retardation](../images/diagrams/8-ship-design/8.5-meson-retardation.svg)
...
- Lower base damage than equivalent lasers, but maintains full damage at all ranges (no falloff) \hyperlink{ref-8.5-db-particle-damage}{[20]}
...
- Available from game start (Particle Beam Strength 2 and Range 60,000 km are starting technologies) \hyperlink{ref-8.5-db-particle-starting}{[21]}
...
| 6,250 | 10% of weapon size |
| 8,000 | 10% of weapon size |
| 10,000 | 10% of weapon size |
| 12,500 | 10% of weapon size |
| 16,000 | 10% of weapon size |
| 20,000 | 10% of weapon size |
...
\hypertarget{ref-8.5-db-gauss-rof}{[9]}. AuroraDB.db FCT_TechSystem: Gauss Cannon Rate of Fire (TechTypeID 141) -- ROF 2 (starting tech, TechSystemID 26608) through ROF 8 (TechSystemID 26630). Six tiers: 2, 3, 4, 5, 6, 8
...
\hypertarget{ref-8.5-db-turret-tracking}{[19]}. AuroraDB.db FCT_TechSystem: Turret Tracking Speed (TechTypeID 75) -- 1,250 km/s through 25,000 km/s, 12 tiers (1250, 2000, 3000, 4000, 5000, 6250, 8000, 10000, 12500, 16000, 20000, 25000), all at 10% gear size

\hypertarget{ref-8.5-db-particle-damage}{[20]}. Aurora C# game database (AuroraDB.db v2.7.1) -- FCT_ShipDesignComponents: 10cm Laser (ComponentTypeID=15) at 3.0 HS has DamageOutput=3; Particle Beam-2 (ComponentTypeID=30) at 6.0 HS has DamageOutput=2. Particle beams have lower base damage per HS than lasers.

\hypertarget{ref-8.5-db-particle-starting}{[21]}. Aurora C# game database (AuroraDB.db v2.7.1) -- FCT_TechSystem: Particle Beam Strength 2 (TechSystemID 25050) StartingSystem=1; Particle Beam Range 60,000 km (TechSystemID 25057) StartingSystem=1. Both are starting technologies available from turn 1.
