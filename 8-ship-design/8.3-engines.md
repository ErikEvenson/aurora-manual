...
![](../images/art/section-art/8.3.png)
...
Engine power comes from a series of technologies in the Power and Propulsion research category (see [Section 7.4.8 Power and Propulsion](../7-research/7.4-tech-categories.md)). Each new technology level provides a higher base Engine Power per Hull Space (EP/[HS](../appendices/B-glossary.md#hs)), making ships faster without requiring larger engines.

The C# version includes additional early-level technologies compared to VB6 Aurora to smooth out the speed progression in the early game. The Conventional Engine has a base power of 1.0 EP/HS \hyperlink{ref-8.3-1}{[1]}; lower effective power values (down to 0.1 EP/HS) are achieved by reducing the power modifier, allowing pre-[TN](../appendices/B-glossary.md#tn) civilizations to field slow but functional spacecraft.
...
- **Military engines:** Higher power per ton, smaller size for equivalent thrust, but cost more minerals and [BP](../appendices/B-glossary.md#bp) (build points). Used on warships, scouts, and any vessel that needs performance. Subject to normal maintenance requirements.
...
**Tip:** Research engine technology as a high priority. Each tier increases the previous tier's output by roughly 25%. A single engine tech advance makes every ship design in your fleet significantly more capable — existing ships can be refitted with new engines, and new designs benefit immediately.
...
![Figure 8.3: Create Research Project — Engine Design](../images/screenshots/8-ship-design/8.3-create-engine.png)
...
![Engine Power Modifier Effects](../images/diagrams/8-ship-design/8.3-power-modifier-curve.svg)

*Figure 8.3.4: The engine power modifier affects speed linearly but fuel consumption exponentially. Low power settings (0.5x) dramatically improve range despite slower speed, while high power settings (2.0x+) cause range to collapse as fuel consumption explodes.*
...
![Engine Power Modifier: Speed vs Range Trade-off](../images/charts/8-ship-design/8.3.5-engine-power-modifier-tradeoffs.png)
...
Power plants are covered comprehensively in [Section 8.6.14 Power Plants](8.6-other-components.md), including the power output formula, boost technology, explosion risks, size selection, and the power allocation priority system.
...
**Tip:** For beam-heavy warships, consider using one large power plant rather than several small ones for space efficiency, but pair it with backup plants for redundancy. See [Section 8.6.14 Power Plants](8.6-other-components.md) for detailed design guidance.
