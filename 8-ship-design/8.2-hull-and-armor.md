...
|---|---|---|
| Conventional I | 2 | Starting (Pre-TN) |
| Conventional II | 2.5 | Starting (Pre-TN) |
| Conventional III | 3 | Starting (Pre-TN) |
| Duranium | 5 | Defensive Systems (TN) |
...
**Tip:** For capital ships, invest in maximum shield generator size research early. A single 50 HS shield provides 224% of the protection of a 10 HS unit while only taking 5x the space — that is a 44% space efficiency bonus. However, pair it with at least one backup generator so that losing a single component does not strip all shielding. For small combatants limited to 10 HS shields, consider whether the tonnage might be better spent on armor, which cannot be knocked out by a single lucky hit.
