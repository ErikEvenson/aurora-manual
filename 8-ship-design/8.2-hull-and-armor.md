...
> **v2.8.0+:** The Fighter/FAC distinction has been unified into a single **Light Naval** classification. The planetary interaction limit is raised from 500 tons to 1,000 tons. Fighter Factories are renamed to Light Naval Factories. The Fighter Combat bonus is replaced by Light Tactical.
...
Duranium Armour becomes available once Trans-Newtonian Technology is researched, marking the transition from conventional to advanced armour systems. Formally, Duranium requires Conventional Advanced Composite Armour as a prerequisite, but since all three conventional armour technologies are starting technologies (always pre-researched), this prerequisite is always satisfied at game start.
...
- Cost: 1 [BP](../appendices/B-glossary.md#bp) per unit (approximately 5% of the cost of normal armour in terms of strength) \hyperlink{ref-8.2-3}{[3]}
...
| Particle Beams | 2 | 1, 3, 5, 7, 5, 3, 1 |
| Lasers | 3-4 | 3, 6, 8, 5, 3 |
...
- **Meson Weapons:** Bypass shields entirely (IgnoreShields=1), but do not ignore armor. Each armor layer has a 40% chance of stopping a meson hit (ArmourRetardation=0.4). If the hit passes through all layers, it deals 1 damage to a random internal component.

A gradient of 1 (missiles, carronades, ramming) produces the widest spread pattern, distributing damage across many columns with a gentle pyramid shape. A gradient of 2 (railguns, particle beams) concentrates damage into fewer columns with steeper falloff. A gradient of 3-4 (lasers, varying by size) focuses damage into a tight cluster, making lasers particularly effective at penetrating individual armour columns *(unverified — the exact size threshold separating gradient 3 from gradient 4 is not exposed in AuroraDB; the gradient is hardcoded in the game engine)*.
...
- Shield generator HTK = floor(SQRT(size in HS)) — one large generator is more space-efficient but a single point of failure
...
**Tip:** For capital ships, invest in maximum shield generator size research early. A single 50 HS shield provides 224% of the protection of five 10 HS shields occupying the same total 50 HS of space (due to square root scaling). For small combatants limited to 10 HS shields, consider whether the tonnage might be better spent on armor, which cannot be knocked out by a single lucky hit. See [Section 8.6.13 Shields](8.6-other-components.md) for detailed shield design guidance.
...
- [Appendix A: Formulas](../appendices/A-formulas.md) -- Armor damage formulas and shield strength calculations
