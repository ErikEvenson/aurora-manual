...
PPV is a defensive metric calculated from the total weapon sizes installed on ships present in a system. Colonies with populations exceeding 10 million require a minimum PPV coverage from ships or Planetary Defence Centres (PDCs) in-system *(unverified — [#723](https://github.com/ErikEvenson/aurora-manual/issues/723) -- the 10 million population threshold is community-reported and not directly confirmed in AuroraDB)* -- without sufficient PPV, the colony is considered unprotected and vulnerable to bombardment.
