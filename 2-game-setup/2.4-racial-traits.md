...
- The exact number of creation points and per-trait costs are not exposed in the game database; the system uses an internal point budget visible only on the race creation screen *(unverified -- exact point pool and cost curve not confirmed in DB)*
...
The political system (government type) is selected during race creation and affects multiple aspects of gameplay \hyperlink{ref-2.4-7}{[7]}. Aurora includes 38 government types, each with distinct modifiers. See [Section 2.2.4 Government Type](2.2-race-creation.md#224-government-type) for the full list.

**Gameplay Effects:**

- **Starting installation ratios**: Different governments receive different proportions of labs, mines, factories, refineries, and shipyards \hyperlink{ref-2.4-7}{[7]}
- **Behavioral modifiers**: Each government applies modifiers to Xenophobia, Diplomacy, Militancy, Expansionism, Determination, and Trade -- affecting NPR AI behavior and diplomatic interactions \hyperlink{ref-2.4-7}{[7]}
- **Training level**: Military governments (Military Dictatorship, Military Theocracy) start with higher training levels (up to 3) \hyperlink{ref-2.4-7}{[7]}
- **Growth and construction modifiers**: Most governments use 1.0 (no modifier), but the Hive Mind government type has a 1.5 growth modifier \hyperlink{ref-2.4-7}{[7]}

| Government Type | Key Modifier | Notes |
|----------------|-------------|-------|
| Player Race | No modifiers | Balanced defaults |
| Representative Democracy | Diplomacy +10, Trade +15 | Diplomatic focus |
| Military Dictatorship | Militancy +20, Training 3 | Military focus |
| Meritocracy | Diplomacy +20, Xenophobia -10 | Research focus |
| Corporate Government | Trade +20, Diplomacy +10 | Commerce focus |
| Hive Mind | Growth 1.5x, Xenophobia +30 | Population focus |
...
5. Configure government type, naming themes, and starting installations separately

> **Note:** The number and names of available species templates are defined in the game's internal configuration. The database includes spoiler race species (Eldar, Precursor, Ancients, Invaders) which are not available as player race templates -- they are used internally by the game for spoiler factions \hyperlink{ref-2.4-8}{[8]}.
...
> **Tip:** For a first game, use the default Human species template and the "Player Race" government type. This provides balanced defaults that do not penalize any particular playstyle.
...
\hypertarget{ref-2.4-7}{[7]}. Aurora C# game database (AuroraDB.db v2.7.1) -- GovType table contains 38 government types with columns: Labs, Construction, Mines, Refineries, Shipyards, Sensors (starting installation ratios), XenophobiaMod, DiplomacyMod, MilitancyMod, ExpansionismMod, DeterminationMod, TradeMod (behavioral modifiers), GrowthMod, ConstMod, TrainingLevel. Player Race (ID 1) has all modifiers at 0/1.0. Hive Mind (ID 27) has GrowthMod=1.5. Military Dictatorship (ID 10) has TrainingLevel=3.

\hypertarget{ref-2.4-8}{[8]}. Aurora C# game database (AuroraDB.db v2.7.1) -- FCT\_Species table contains 5 species: Human, Eldar, Precursor, Ancients, Invaders. Human species defaults used for player races; others are spoiler faction species
