...
- The exact number of creation points and per-trait costs are not exposed in the game database; the system uses an internal point budget visible only on the race creation screen *(unverified — #702 -- exact point pool and cost curve not confirmed in DB)*
...
| Officer Generation Rate | 1.0 *(unverified — #702)* | Multiplier on commander generation frequency |
