...
Aurora C# does not provide a built-in option to start in a system other than Sol. However, experienced players can use SpaceMaster mode after game creation to set up a custom starting location:

**Manual Custom Start Process:**

1. Create a new game with default settings (you will start in Sol)
2. Enable SpaceMaster mode (Ctrl+S)
3. Use "Create System" to generate a random star system
4. Find a suitable homeworld planet in the new system
5. Use SM tools to create a colony and add starting installations
6. Optionally delete or ignore the Sol-based starting empire

This is an advanced workaround, not a supported game feature. It requires significant familiarity with SpaceMaster tools (see [Section 18.5 SpaceMaster Mode](../18-advanced-topics/18.5-spacemaster-mode.md)).

**Advantages of Custom Start:**

- **Unique Geography**: Each playthrough presents new challenges and opportunities
- **Potentially Better Resources**: Random generation might give you rich mineral deposits or convenient gas giant placement
- **Replayability**: No two custom starts are alike

**Disadvantages of Custom Start:**

- **Manual Setup Required**: No automated "random start" option exists
- **Complex Process**: Requires understanding SpaceMaster mode before playing
- **No Reference Frame**: New players lose the intuitive distance sense that Sol provides
- **Tutorial Disconnect**: Guides referencing Sol features will not apply

> **Note:** This approach is not recommended for new players. Learn the game with a standard Sol start first.
...
\hypertarget{ref-2.5-10}{[10]}. Aurora C# does not include a "SpaceMaster Empire" starting option — this was a VB6-era feature. Custom starting locations in C# require manual SpaceMaster setup after game creation.
