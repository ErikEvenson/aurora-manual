...
![](../images/art/section-art/2.2.png)

![Figure 2.2: Create New Race Window](../images/screenshots/2-game-setup/2.2-race-creation.png)
...
- **Non-Player Race**: Designates whether the race is controlled by the AI or the player \hyperlink{ref-2.2-8}{[8]}. When enabled, the race becomes an [NPR](../appendices/B-glossary.md#npr) (Non-Player Race) with autonomous AI behavior. When disabled, you control the race directly.

- **Industrial (Low Tech)**: Sets the race to start with pre-industrial or basic conventional technology levels with minimal infrastructure \hyperlink{ref-2.2-8}{[8]}. When disabled (recommended for most players), the race starts with [Trans-Newtonian (TN)](../appendices/B-glossary.md#tn) technology already researched.
  ...
  **Conventional Start** (checkbox enabled):
  ...
  - Adds 20-50 years of gameplay before you are truly spacefaring *(unverified — [#1285](https://github.com/ErikEvenson/aurora-manual/issues/1285) -- community estimate)*
...
- **No Background Research** *(v2.8.0)*: When enabled, disables automatic background technology research for this race \hyperlink{ref-2.2-20}{[20]}.

  **Important:** This setting is **locked at race creation** and cannot be modified afterward. Unlike most game options, this is a permanent choice that defines how the race develops throughout the entire game.

  **Background Research Mechanic:** By default, races automatically research certain foundational technologies in the background over time, even without explicit scientist assignments. This represents ongoing technological progress from general scientific advancement. Disabling background research forces the race to rely entirely on explicit research project assignments for all technological advancement.

  **SpaceMaster Mode Interaction:** Even when No Background Research is enabled, SpaceMaster Mode's instant research feature still functions on background technologies \hyperlink{ref-2.2-20}{[20]}. This allows scenario designers and referees to grant technologies that would otherwise be inaccessible.

  **Strategic Implications:**

  - Races with No Background Research enabled will fall behind technologically over time unless they maintain active research programs
  - Particularly impactful for NPRs, which may become progressively weaker relative to races with background research enabled
  - Creates asymmetric gameplay scenarios where some races have inherent technological advantages
  - Useful for roleplaying isolated or stagnant civilizations

  > **Warning:** Enabling this option for player races significantly increases the research management burden. Every technology must be explicitly researched -- nothing comes "for free" over time. This is generally recommended only for experienced players seeking additional challenge or for creating specific scenario conditions.
...
![Figure 2.2.6a: Installation Production Chain](../images/diagrams/2-game-setup/2.2-installation-production-chain.svg)

![Figure 2.2.6b: Installation Trade-offs Comparison](../images/charts/2.2-installation-tradeoffs.png)

Starting installations and their production relationships:
...
- **Fighter Factories**: Produce light naval craft up to 500 tons *(v2.7.1)* / 1,000 tons *(v2.8.0+)* (10 BP/year) \hyperlink{ref-2.2-6}{[6]}.
...
![Figure 2.2.7: Government Type Decision Tree](../images/diagrams/2-game-setup/2.2-government-decision-tree.png)
...
![Figure 2.2.7: Government Type Impact Matrix](../images/diagrams/2-game-setup/2.2-government-impact-matrix.svg)
...
\hypertarget{ref-2.2-20}{[20]}. Aurora Forums, "v2.8.0 Discussion Thread" -- [aurora2.pentarch.org](https://aurora2.pentarch.org/index.php?topic=13894.0). No Background Research option is locked at race creation and cannot be modified afterward. SpaceMaster Mode's instant research feature still functions on background technologies regardless of this restriction.
