...
As of v2.0.0, there are 38 distinct terrain types. The original 24 were expanded by 14 new types to better represent planets with significant orbital temperature variation:
...
| Forest | 1.0 | 0.5 | Moderate concealment |
...
| Rift Valley | 2.0 | 0.5 | Broken terrain similar to mountain |
| Barren | 0.5 | 1.0 | Minimal concealment |
...
- **Direct Commander:** Provides 100% of their ground combat bonus (GCO, GCD, GCA, etc.) to the formation they directly command
- **Superior Commander (one level up):** Provides 25% of their bonus to formations commanded by their subordinate commanders
...
Each attack can hit multiple targets within a formation. The number of sub-attacks per attack equals 50 / target size.
...
- Each NBG shot has a one-third chance to also strike the population itself, inflicting installation damage and population losses
- Conversely, planetary bombardment attacks (targeting population/installations) have a one-third chance to hit any ground forces present, regardless of detection status
...
The fire control features a 25% range bonus versus ship-mounted equivalents. Cost and size are 50% of the ship version due to dedication to a single weapon (uses the single weapon fire control system).

**Active Sensor:** Resolution-1 with minimum 5-ton size, range matching the weapon's maximum range.

**Reactor:** Generates sufficient power for the weapon capacitor at normal cost/size.

**ECCM:** Optional 50-ton component at half normal cost.
...
General orbital bombardment allows ships in orbit to fire on enemy surface positions to soften defenses, reduce fortification levels, or support friendly forces. This differs from the precision support in Section 13.3.8 and the mass NBG bombardment in Section 13.3.10.
...
- **Population Casualties:** General bombardment kills civilians at approximately **100,000 civilians per point of warhead strength**. Even a modest missile salvo can devastate a colony's population, making orbital bombardment an extremely blunt instrument against inhabited worlds.
...
- Combat rounds occur every **300 seconds (5 minutes)**
- **Defender fortification bonus:** Defenders receive a fortification value of **level 2**, representing dug-in positions in corridors, choke points, and makeshift barricades. Attackers have a base fortification of **level 1** (before any commander GCD bonuses). This gives defenders a significant accuracy reduction against incoming fire without needing time to fortify.
...
If a boarding shuttle can reach the target ship within **5 seconds** of deployment, the troops will deploy before the shuttle can be intercepted and destroyed. This makes boarding viable even against ships with point defense, provided the boarding vessel is close enough at the moment of launch. Plan your approach to close within this 5-second transfer window before committing marines.
...
- Captured ship receives a **"01 status"** -- an overhaul factor of 0.01 (essentially requires complete overhaul), indicating the ship needs full crew replacement and extensive repair (see [Section 14.2 Maintenance](../14-logistics/14.2-maintenance.md))
