...
![](../images/art/section-art/13.3.png)
...
![Ground Combat Phase Sequence](../images/diagrams/13-ground-forces/13.3-combat-phase-sequence.svg)
...
![Terrain Effects on Ground Combat](../images/charts/13-ground-forces/13.3.1-terrain-fortification-modifiers.png)
...
- **Forfeits all fortification bonuses** while in attack mode *(unverified — #747)*
- Generates doubled morale gains from kills *(unverified — #747)*
- Only fully-supplied formations may assume this position *(unverified — #747)*
...
- Fires simultaneously with orbital bombardment *(v2.8.0+: ground support fighters removed)*
...
**No Targeting Priority Control:** *(unverified — [#866](https://github.com/ErikEvenson/aurora-manual/issues/866))*
...
*(unverified — #747)* \hyperlink{ref-13.3-10}{[10]}
...
**Ground Commander Skill Distinctions:** *(unverified — [#866](https://github.com/ErikEvenson/aurora-manual/issues/866))*
...
- **Direct Commander:** Provides 100% of their ground combat bonus (GCO, GCD, GCA, etc.) to the formation they directly command *(unverified — #747)*
- **Superior Commander (one level up):** Provides 25% of their bonus to formations commanded by their subordinate commanders *(unverified — #747)*
...
- When breakthrough potential >= 30%, attacking formations gain **additional attacks** *(unverified — #747)*
...
All elements move from non-fortified to their maximum self-fortification level in **30 days** without outside assistance: *(unverified — #747)*
...
- Given sufficient capacity, a construction element can fortify any other element from its self-fortification level to maximum fortification in **90 days** *(unverified — #747)*
- Construction capacity = construction rating x unit count x racial construction rating x commander bonus x 100 tons *(unverified — #747)*
...
- Moving/loading a formation resets fortification to 0 *(unverified — #747)*
- Taking casualties does NOT reduce fortification level *(unverified — #747)*
...
> **v2.8.0+:** Ground support fighters have been removed and are no longer an option for breaking fortifications.
...
- Ground forces are treated as **size-1** for detection purposes *(unverified — #747)*
...
- Each vessel fires **once per ground combat phase** *(unverified — #747)*
- Cannot fire if the ship fired during the preceding naval combat phase in the same increment *(unverified — #747)*
- Uses same accuracy system as ground unit bombardment

### 13.3.8.3 Accuracy Modifiers

- Modified by crew grade and morale
- Tactical officer contributes **100%** of their ground support bonus *(unverified — #747)*
- Ship commander provides **50%** of their bonus *(unverified — #747)*
...
- Each formation FFD component enables **1 orbital bombardment ship** \hyperlink{ref-13.3-6}{[6]} *(v2.8.0+: ground support fighters removed; prior versions also allowed 6 fighters per FFD)*
...
Each attack can hit multiple targets within a formation. The number of sub-attacks per attack equals 50 / target size. *(unverified — #747)*
...
- Each NBG shot has a one-third chance to also strike the population itself, inflicting installation damage and population losses *(unverified — #747)*
- Conversely, planetary bombardment attacks (targeting population/installations) have a one-third chance to hit any ground forces present, regardless of detection status *(unverified — #747)*
...
The fire control features a 25% range bonus versus ship-mounted equivalents. Cost and size are 50% of the ship version due to dedication to a single weapon (uses the single weapon fire control system). *(unverified — #747)*

**Active Sensor:** Resolution-1 with minimum 5-ton size, range matching the weapon's maximum range. *(unverified — #747)*

**Reactor:** Generates sufficient power for the weapon capacitor at normal cost/size. *(unverified — #747)*

**[ECCM](../appendices/B-glossary.md#eccm):** Optional 50-ton component at half normal cost. *(unverified — #747)*

### 13.3.11.2 STO Targeting Options

Ground elements with STO capability can select from these targeting modes:

- Target random/largest/smallest/fastest/slowest/easiest ship — when set to "random," the targeting roll granularity (whether the entire formation fires at one randomly selected ship, or each element independently selects a random target) is unknown *(unverified — #1359)*
...
- **Population Casualties:** General bombardment kills civilians at approximately **100,000 civilians per point of warhead strength**. *(unverified — #747)* Even a modest missile salvo can devastate a colony's population, making orbital bombardment an extremely blunt instrument against inhabited worlds.
...
As of v2.0.0, some Spoiler and [NPR](../appendices/B-glossary.md#npr) (Non-Player Race) factions will avoid bombing populations except when specifically targeting Surface-To-Orbit (STO) weapons. This behavior is influenced by the race's xenophobia setting. Low-xenophobia races are more likely to restrict their bombardment to military targets only, while highly xenophobic races may still employ indiscriminate bombardment.
...
[Planetary Defence Centres (PDCs)](../appendices/B-glossary.md#pdc) interact with ground combat in specific ways that differ from open-field engagements. Understanding these mechanics is essential when planning assaults on fortified worlds.
...
- Combat rounds occur every **300 seconds (5 minutes)** *(unverified — #747)*
- **Defender fortification bonus:** Defenders receive a fortification value of **level 2**, representing dug-in positions in corridors, choke points, and makeshift barricades. Attackers have a base fortification of **level 1** (before any commander GCD bonuses). *(unverified — #747)* This gives defenders a significant accuracy reduction against incoming fire without needing time to fortify.
...
- Crew defense has **armor equal to half the lowest racial armor for infantry** *(unverified — #747)*
...
If a boarding shuttle can reach the target ship within **5 seconds** of deployment, the troops will deploy before the shuttle can be intercepted and destroyed. *(unverified — #747)* This makes boarding viable even against ships with point defense, provided the boarding vessel is close enough at the moment of launch. Plan your approach to close within this 5-second transfer window before committing marines.
...
- Captured ship receives a **"01 status"** -- an overhaul factor of 0.01 (essentially requires complete overhaul) *(unverified — #747)*, indicating the ship needs full crew replacement and extensive repair (see [Section 14.2 Maintenance](../14-logistics/14.2-maintenance.md))
...
3. **Bombardment Phase:** Support elements and orbital bombardment ships fire *(v2.8.0+: ground support fighters removed)*
4. **Direct Combat Phase:** Front-line elements engage selected targets
5. **AA Phase:** Anti-aircraft elements engage attacking aircraft *(v2.8.0+: no longer applicable with ground support fighters removed)*
