...
- **Orbital Bombardment Calculation:** The Tactical Officer contributes 100% of their bonus; the Ship Commander contributes 50% of their bonus *(unverified)*
...
- Maximum achievable morale = 100 + (5 x training bonus percentage) *(unverified)*
...
Formations receive a replacement priority value (default: 10) that determines reinforcement order: *(unverified)*
...
- **Cost:** 12.5% of Ground Unit build cost per annum as a wealth expense *(unverified)*
...
Each ground unit carries inherent supply sufficient for **10 combat rounds**. After exhausting inherent supply: *(unverified)*

- Units fire at only **25% effectiveness** *(unverified)*
- Units **cannot** occupy "Front Line Attack" positions *(unverified)*
...
- **Morale Decay Rate:** Troops in non-cryogenic drop modules lose **1 morale point per day** while loaded. *(unverified)* A formation starting at 100 morale would drop to 80 morale after 20 days in standard drop modules.
- **Combat Effectiveness Threshold:** Since combat effectiveness scales linearly with morale (Strength x Morale/100), troops at 50 morale fight at only 50% effectiveness. Morale cannot drop below 25 under normal circumstances *(unverified)* (see [Section 16.2 Skills and Bonuses](../16-commanders/16.2-skills-and-bonuses.md)).
