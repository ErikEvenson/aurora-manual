...
**Fighter Factories (Light Naval Factories):**

Fighter Factories (renamed to Light Naval Factories as of v2.8.0) are specialised installations that produce Light Naval vessels -- any military ship of 1,000 tons or less. They function similarly to construction factories but are dedicated to small craft production. See [Section 9.6 Light Naval Operations](../9-fleet-management/9.6-light-naval-operations.md) for full details on Light Naval construction and mechanics.
...
At game start, empires begin with conventional industry representing pre-TN (trans-newtonian) manufacturing capability. Conventional Industry (CI) is **immobile** -- it cannot be transported between colonies or relocated in any way. It exists only on the body where your empire starts. *(v2.7.1)*

After researching Trans-Newtonian Technology, the Industry tab shows conversion options for all major TN installation types, not just a single type. Note that CI-to-TN conversion requires the Trans-Newtonian Technology research to unlock; without it, no conversion options appear. *(v2.7.1)*

**CI vs TN Efficiency -- The 1:10 Mining Ratio:** *(v2.7.1)*

New players should understand the dramatic efficiency gap between conventional and TN industry:

| Facility | Annual Output | Notes |
|----------|---------------|-------|
| 1 Conventional Industry unit | ~1 ton of minerals/year | Pre-TN extraction |
| 1 TN Mine | 10 tons of minerals/year | Base rate before tech upgrades |

This is a **1:10 efficiency ratio** -- a single TN mine produces ten times the mineral output of a single CI unit. Prioritize transitioning to TN mines as early as possible; every year spent relying solely on CI is a year of drastically reduced mineral income. The compound effect of early TN conversion cannot be overstated.
...
Each conversion costs 1/16th of the normal build cost for that installation type. Conversions can be queued simultaneously with percentage-based priority allocation.
