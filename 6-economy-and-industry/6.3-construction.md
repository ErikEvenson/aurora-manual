...
Each construction factory contributes a set number of Build Points (BP) per year toward whatever is in the colony's construction queue. The base output is 10 BP per factory per year \hyperlink{ref-6.3-1}{[1]}, improvable through technology research.
...
- **Build Rate:** Shipyard output is measured in BP/year, similar to construction factories. The base rate is 400 BP per slipway per year (modifiable by technology). *(unverified -- 400 BP/slipway/year not directly confirmed in DB)*
...
Fighter Factories (renamed to Light Naval Factories as of v2.8.0) are specialised installations that produce Light Naval vessels -- any military ship of 1,000 tons or less *(unverified -- 1,000-ton threshold not directly confirmed in DB)*. They function similarly to construction factories but are dedicated to small craft production. See [Section 9.6 Light Naval Operations](../9-fleet-management/9.6-light-naval-operations.md) for full details on Light Naval construction and mechanics.
...
Each conversion costs 20 BP (1/6th of the normal 120 BP build cost for most installation types) \hyperlink{ref-6.3-7}{[7]}. Conversions can be queued simultaneously with percentage-based priority allocation.
...
- Begin with a baseline capacity of 10,000 tons *(unverified -- no repair yards in DB to confirm default capacity)*
...
\hypertarget{ref-6.3-7}{[7]} AuroraDB.db: DIM_PlanetaryInstallation CI conversion entries (IDs 27, 28, 30, 36, 37, 50) all show Cost=20 BP. Target installations cost 120 BP each, making conversion cost 20/120 = 1/6 of the normal build cost. Previous text incorrectly stated 1/16.

\hypertarget{ref-6.3-8}{[8]} Verified mathematically: SQRT(10 * 5000) = 223.6 (rounds to 224 RP for a 10 BP component); SQRT(1000 * 5000) = 2236 RP for a 1000 BP component. Both match the manual's worked examples. Formulas sourced from v1.13.0 changelog.

\hypertarget{ref-6.3-9}{[9]} Verified mathematically: Standard breakeven at 50 BP (old: 50*10=500, new: SQRT(50*5000)=500); missile breakeven at 2.5 BP (old: 2.5*100=250, new: SQRT(2.5*25000)=250); ground unit breakeven at 10 BP (old: 10*50=500, new: SQRT(10*25000)=500). HQ example: SQRT(501*5000) = 1582.7 (rounds to 1,583 RP), matching claimed value.
