...
**Starting Financial Centres:** New Trans-Newtonian races start with a number of Financial Centres equal to one quarter of their starting Construction Factories. For example, a race beginning with 100 Construction Factories automatically receives 25 Financial Centres. This ensures new races have adequate financial infrastructure proportional to their industrial capacity without requiring manual setup. *(unverified -- starting FC ratio not directly confirmed in DB)*
...
- **Cap Formula:** Maximum Wealth = 2 x Annual Wealth Income *(unverified -- cap formula not directly confirmed in DB)*
...
- Tax rate is adjustable per colony via the Economics window *(unverified -- per-colony tax controls not directly confirmed in DB)*
- The tax rate range and default value are not exposed in the game database; they are controlled through the in-game UI *(unverified)*
- Very high tax rates reduce population growth and can trigger unrest *(unverified)*
- Very low tax rates boost morale and growth but reduce income *(unverified)*
...
- **Ship Maintenance:** Every active ship in your fleet (see [Section 9.4 Fleet Organization](../9-fleet-management/9.4-fleet-organization.md)) costs wealth to maintain, representing crew salaries, consumables, and routine maintenance *(unverified)*. Larger and more advanced ships cost more *(unverified)*. This is typically the largest single wealth expense *(unverified)*.
- **Installation Maintenance:** Ground installations have ongoing wealth maintenance costs, though these are generally smaller per-unit than ship costs *(unverified)*.
- **Crew Training:** Training ground forces and replacing crew casualties costs wealth *(unverified)*.
- **Terraforming:** Some terraforming operations consume wealth in addition to minerals *(unverified)*.
- **Espionage and Intelligence:** Running intelligence operations against other races costs wealth *(unverified)*.
- **Ordnance Production:** Missiles and other ordnance have wealth costs in addition to mineral costs *(unverified)*.

**Maintenance Cost Reduction:**

- The "Reduced Maintenance" technology line reduces fleet maintenance costs by a percentage per tech level *(unverified)*. This is a high-priority research topic for large fleets.
- Mothballing ships (placing them in reserve) significantly reduces their maintenance costs but makes them unavailable for immediate use *(unverified)*.
...
If your wealth reserves reach zero and you are running a deficit *(unverified)*:

- Fleet maintenance may not be fully paid, leading to reduced readiness *(unverified)*.
- Construction and research may be impacted *(unverified)*.
- Population morale can suffer *(unverified)*.
- In extreme cases, ships may become inactive due to unpaid crews *(unverified)*.

**Production Penalty from Wealth Deficit:**

When an empire's wealth balance goes negative, a production penalty is applied proportionally across all colonies *(unverified)*. Industrial output from Construction Factories, Ordnance Factories, Fighter Factories, and Research Labs is reduced based on the severity of the deficit relative to annual wealth income *(unverified)*. The deeper the deficit, the greater the penalty -- effectively throttling your empire's entire industrial capacity until the shortfall is resolved. This mechanic uses the same proportional reduction pattern as unrest penalties *(unverified)* (see [Appendix A: Formulas, Section A.5](../appendices/A-formulas.md) for the general production modifier structure).
...
**Performance:** In developer testing, routing 364 civilian freighters across 495 star systems with 345 populations required only 1.01 seconds, creating 1,896 individual movement orders including Lagrange point shortcuts. *(unverified -- performance figures from developer changelog, not directly confirmed in DB)*
...
**Cost = 5 x Installations x Systems Travelled x (Cargo Points / 25,000)** *(unverified -- formula from changelog, not directly confirmed in DB)*
...
Each trade good occupies 2,500 tons of cargo space (one-tenth of a standard cargo hold), allowing freighters to carry multiple goods per trip. *(unverified -- 2,500 ton cargo size not directly confirmed in DB)*

### Supply and Demand Formulas

Trade good production and consumption are governed by:

**Supply per year** = Population (millions) x 2 x (Wealth Creation Tech / 100) x Governor Bonus x Random Factor (10%-300%) *(unverified -- formula from changelog)*

**Demand per year** = Population (millions) x 3 x Governor Bonus *(unverified -- formula from changelog)*

Infrastructure always uses a 100% random factor (no variance), ensuring predictable infrastructure supply. *(unverified)*
...
*(unverified -- trade revenue rates from changelog, not directly confirmed in DB)*

Shipping lines receive fixed wealth for their services: 1 wealth per good per jump in C# (0.5 for same-system deliveries). *(unverified)*
