...
**Starting Financial Centres:** New Trans-Newtonian races start with a number of Financial Centres equal to one quarter of their starting Construction Factories. For example, a race beginning with 100 Construction Factories automatically receives 25 Financial Centres. This ensures new races have adequate financial infrastructure proportional to their industrial capacity without requiring manual setup. *(unverified — #837 -- requires live testing: ratio is game startup logic not stored in DB)*
...
- Tax rate is adjustable per colony via the Economics window *(unverified — #837 -- requires live testing: UI controls not stored in DB)*
- The tax rate range and default value are not exposed in the game database; they are controlled through the in-game UI *(unverified — #837 -- requires live testing)*
- Very high tax rates reduce population growth and can trigger unrest *(unverified — #837 -- requires live testing: tax effect mechanics not stored in DB)*
- Very low tax rates boost morale and growth but reduce income *(unverified — #837 -- requires live testing)*
...
- Mothballing ships (placing them in reserve) significantly reduces their maintenance costs but makes them unavailable for immediate use *(unverified — #837 -- requires live testing: MaintenanceState column exists but reduction percentage not stored in DB)*.
...
**Cost = 5 x Installations x Systems Travelled x (Cargo Points / 25,000)** *(unverified — #837 -- requires live testing: formula from changelog, game logic not stored in DB)*
...
Each trade good occupies 2,500 tons of cargo space (one-tenth of a standard cargo hold), allowing freighters to carry multiple goods per trip. *(unverified — #837 -- requires live testing: cargo size is game logic not stored in DIM_TradeGoods)*

### 6.4.3.6 Supply and Demand Formulas

Trade good production and consumption are governed by:

**Supply per year** = Population (millions) x 2 x (Wealth Creation Tech / 100) x Governor Bonus x Random Factor (10%-300%) *(unverified — #837 -- requires live testing: formula from changelog, game logic not stored in DB)*

**Demand per year** = Population (millions) x 3 x Governor Bonus *(unverified — #837 -- requires live testing: formula from changelog)*

Infrastructure always uses a 100% random factor (no variance), ensuring predictable infrastructure supply. *(unverified — #837 -- requires live testing)*
...
*(unverified — #837 -- requires live testing: trade revenue rates from changelog, game logic not stored in DB)*

Shipping lines receive fixed wealth for their services: 1 wealth per good per jump in C# (0.5 for same-system deliveries). *(unverified — #837 -- requires live testing)*
