...
- Mining output is subtracted directly from the deposit quantity each production cycle (every 5-day increment the game processes). *(unverified — #752 -- 5-day cycle is commonly cited but not directly confirmed in DB)*
- **Accessibility declines during depletion:** For planets and moons, once half of a deposit has been mined (50% depletion), the accessibility begins to decline, eventually reaching a minimum of 0.1 shortly before the deposit is exhausted. *(unverified — #752 -- 50% threshold and 0.1 minimum not directly confirmed in DB)* This means early extraction is significantly more efficient than late-stage mining from the same deposit. See [Section 6.2.1 Conventional Mines](6.2-mining.md) for details on how this affects mining output.
