...
Every mineral deposit on a planetary body has two key values: the total quantity available (in tons) and its accessibility rating (a value from 0.1 to 1.0, displayed as a decimal). *(unverified -- accessibility range not directly confirmed in DB)*
...
**Accessibility Does Not Change:** Once a deposit is surveyed, its accessibility remains constant for the life of the deposit. *(unverified -- this contradicts Section 6.1.3 which states accessibility declines after 50% depletion; the statement here refers to the initial accessibility value prior to depletion effects)* You cannot improve accessibility through technology or infrastructure.
...
- Mining output is subtracted directly from the deposit quantity each production cycle (every 5-day increment the game processes). *(unverified -- 5-day cycle is commonly cited but not directly confirmed in DB)*
- **Accessibility declines during depletion:** For planets and moons, once half of a deposit has been mined (50% depletion), the accessibility begins to decline, eventually reaching a minimum of 0.1 shortly before the deposit is exhausted. *(unverified -- 50% threshold and 0.1 minimum not directly confirmed in DB)* This means early extraction is significantly more efficient than late-stage mining from the same deposit. See [Section 6.2.1 Conventional Mines](6.2-mining.md) for details on how this affects mining output.
...
\hypertarget{ref-6.1-1}{[1]} AuroraDB.db: ComponentType table shows Neutronium used in Railgun components and Shipyard installations, not armour.

\hypertarget{ref-6.1-2}{[2]} AuroraDB.db: ComponentType table shows Corbomite used in Active Sensors, Fire Controls, Cloaking Devices, and Missile Decoys.

\hypertarget{ref-6.1-3}{[3]} AuroraDB.db: ComponentType table shows Tritanium as primary mineral for Missile Launchers, Magazines, and Ordnance Factories.

\hypertarget{ref-6.1-4}{[4]} AuroraDB.db: ComponentType table shows Boronide used in energy weapon components alongside power systems.

\hypertarget{ref-6.1-5}{[5]} AuroraDB.db: InstallationType table shows Mercassium used in Research Facilities and Infrastructure; ComponentType shows no ship sensor usage.

\hypertarget{ref-6.1-6}{[6]} AuroraDB.db: GroundUnitType table shows Vendarite as primary mineral for ground forces; ComponentType shows CIWS and Gauss Cannon usage.

\hypertarget{ref-6.1-7}{[7]} AuroraDB.db: ComponentType table shows Sorium as major component in Jump Drive construction.

\hypertarget{ref-6.1-8}{[8]} AuroraDB.db: ComponentType table shows Uridium as primary mineral for Active Sensors, Passive Sensors, and Fire Control systems.

\hypertarget{ref-6.1-9}{[9]} AuroraDB.db: ComponentType table shows Corundium as primary mineral for Lasers, Particle Beams, Meson Cannons, Plasma Carronades, and HPM weapons.

\hypertarget{ref-6.1-10}{[10]} AuroraDB.db: DIM_PlanetaryInstallation table has 11 mineral columns (Duranium, Neutronium, Corbomite, Tritanium, Boronide, Mercassium, Vendarite, Sorium, Uridium, Corundium, Gallicite), confirming 11 trans-newtonian minerals.

\hypertarget{ref-6.1-11}{[11]} AuroraDB.db: DIM_PlanetaryInstallation table shows Fuel Refinery requires 120 Boronide and 0 Duranium. Previous text incorrectly stated "Duranium and Boronide."

\hypertarget{ref-6.1-12}{[12]} AuroraDB.db: DIM_PlanetaryInstallation "Convert CI to Financial Centre" (ID 50) costs 20 BP and requires 20 Corbomite. Note: the conversion is from Conventional Industry (CI), not from Construction Factories.

\hypertarget{ref-6.1-13}{[13]} AuroraDB.db: FCT_TechSystem mining production tech line starts at 10 tons (base) and progresses through 12, 14, 16, 20, 25, 30, 36, 42, 50, 60, 70 tons/year per mine, confirming base output of 10 tons/year.
