...
- Civilian shipping lines begin forming once your empire reaches a certain level of development (typically when you have multiple colonies with significant population) *(unverified -- formation threshold not directly confirmed in DB)*
- New shipping lines appear periodically as your economy grows *(unverified)*
- Each shipping line operates independently, building ships and establishing routes based on economic opportunity
- The number of active shipping lines scales with your empire's overall economic development *(unverified)*
...
*(unverified -- income rates from v2.6.0/v2.7.0 changelog, not directly confirmed in DB)*
...
**Colonist Generation Formula:** *(unverified -- formula from changelog)*
```
Available Colonists = (Construction Phase Length / Year) * Population * Colonization Pressure
```

**NPR Shipping Lines (v2.7.0):** NPR shipping lines are reduced to one freighter and one colony ship per 200 million population (down from 100 million). Harvesters are removed for NPRs. NPR payment modifier is 0.5x. *(unverified -- NPR shipping rules from changelog)*
...
- **Instant Repair in Orbit:** Civilian ships instantly repair themselves if they are in orbit of a population with shipyards. *(unverified -- instant repair mechanic from changelog)* This abstraction avoids requiring player-owned shipyard facilities or explicit civilian shipyard structures.
...
Civilian ships will only travel a maximum of **four systems** from their origin for deliveries. *(unverified -- range cap from changelog)* This hard range cap affects all civilian logistics -- colonist transport, freight, trade goods, and infrastructure delivery.
...
- Each click of the Subsidize button adds **1,000 wealth** to the selected shipping line's coffers *(unverified -- 1,000 wealth per click from changelog)*
...
- Ship build cost equals the ship's Build Points in wealth, so a 5,000 BP freighter costs 5,000 wealth to construct *(unverified -- wealth-based build cost from changelog)*
...
- Civilian shipping lines build their own ships at commercial shipyards *(unverified)*
- They pay for construction using their own profits (this does not cost you minerals or wealth directly); the build cost equals the ship's Build Points in wealth *(unverified)*
...
- Civilian ships use commercial engines (larger but cheaper than military engines; see [Section 8.3 Engines](../8-ship-design/8.3-engines.md)) and do not carry weapons *(unverified)*
- Civilian ships cannot build jump-capable ships; inter-system civilian shipping requires a Jump Gate network (see [Section 4.4 Jump Points](../4-systems-and-bodies/4.4-jump-points.md)) *(unverified)*
...
- Ships are selected for retirement based on being the oldest ship of the same type, with a minimum age of 10 years *(unverified -- 10-year minimum from changelog)*
- All civilian ships are scrapped when they reach 15 years old, regardless of condition *(unverified -- 15-year retirement age from changelog)*
...
A fleet standing order allows player-controlled fleets to seek and fulfill civilian contracts, effectively turning government ships into supplementary civilian haulers. *(unverified -- standing order mechanics from v2.8.0 changelog)*
...
- To supplement logistics to frontier colonies beyond the civilian four-system range limit
...
- **Refuelling Cost:** Military ships refuelling from civilian harvesters pay 1 wealth per 10,000 litres of fuel *(unverified -- refuelling cost from changelog)*
- **Tax Revenue:** Excess fuel produced by civilian harvesters generates tax revenue at 20% of the fuel price *(unverified -- tax rate from changelog)*
...
**Probability Check:** Each construction phase, the system rolls 1-50,000 against: *(unverified -- probability check from changelog)*
...
- **Minimum Duranium:** At least 10,000 tons of Duranium present *(unverified -- threshold from changelog)*
- **Minimum Accessibility:** At least 0.7 accessibility rating *(unverified -- threshold from changelog)*
- **System Population:** Located in a system with a population of 10 million or more *(unverified -- threshold from changelog)*
- **Distance from Star:** Less than 80 AU from the parent star *(unverified -- distance cap from changelog)*
- **Multi-Star Systems:** If orbiting a non-primary star, that star must be within 80 AU of the primary or connected via Lagrange Points *(unverified)*
...
- The expansion roll is 1-100,000 (half the probability of new colony establishment) *(unverified -- expansion probability from changelog)*
...
In C# Aurora, the threshold for player control over civilian destinations has been lowered to **the lower of 10 million or half the system body's capacity** (down from 25 million in VB6). *(unverified -- threshold from changelog)*

### Automatic vs. Player-Controlled Destinations

**Below the threshold (automatic):**

- Populations function as automatic destinations requiring no player input *(unverified)*
- Civilian shipping lines service these colonies without intervention *(unverified)*
- All trade goods become available at the 10 million population level

**Above the threshold (player-controlled):**

- Players gain control over colony designation options *(unverified -- designation controls from community knowledge)*:
...
- **Commercial Shipyard Priority:** You can prioritize civilian construction orders at your commercial shipyards. Government orders can be set to take priority, or you can allow civilians to use excess capacity. *(unverified -- priority controls from community knowledge)*
