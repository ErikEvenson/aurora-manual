...
![Figure 7.1: Economics -- Research Tab](../images/screenshots/7-research/7.1-research-window.png)
...
After a prototype component is formally researched to completion, the ship class may still display the (P) designation. The Class Design window's **"Refresh Tech"** button forces the design to re-evaluate all component statuses. Clicking Refresh Tech clears the (P) designation from any class whose prototype components have all been fully researched, making the class eligible for shipyard retooling and production.

> **Warning:** If you forget to click Refresh Tech after researching a prototype, the class will not appear in shipyard retool or construction dropdowns. This is a common source of confusion — the class looks complete in the designer but the shipyard cannot see it.
...
5. Once all components are researched, click **Refresh Tech** to clear the (P) designation
6. The class becomes production-ready for shipyard retooling and construction
...
### 7.1.2.1 Prototype-to-Production Pipeline

![Figure 7.1.2.1: Research Proto — Converting Prototype to Research Project](../images/screenshots/7-research/7.1-research-proto.png)

The complete workflow from designing a custom component to building ships involves steps across multiple windows. Missing any step blocks the pipeline.

**Step 1 — Design the component:** Open Class Design (F5), click **"Design Tech!"** to open the Create Research Project window. Select the component type (e.g., Engines), configure parameters, and click **"Prototype"** (for immediate design use) or **"Create"** (to go directly to a research project).

**Step 2 — Add to ship class:** In Class Design, create or select a ship class. Enable the **"Prototypes"** checkbox in the component list to see prototype components. Double-click to add them to the design.

**Step 3 — Convert to research project:** Select the prototype component in the **Class Components** view of the ship design. Click the **"Research Proto"** button that appears. This converts the (P) component to an (RP) Research Prototype and creates a research project in the Economics window.

**Step 4 — Research the component:** Open Economics (F2) > Research tab. The (RP) component appears under its research category. Either assign a scientist for normal research or use Instant Research Points to complete it immediately.

**Step 5 — Refresh the design:** Return to Class Design (F5), select the ship class, and click **"Refresh Tech"**. This clears the (P) designation from the class now that all prototype components are fully researched.

**Step 6 — Retool a shipyard:** Open Economics (F2) > Shipyard tab. Select a shipyard, change Task Type to **"Retool"**, select the class from the dropdown, and click **"Set Activity"**. The class appears in the Retool dropdown, not the Construction dropdown, until retooling is complete.

**Step 7 — Build ships:** After retooling completes, change Task Type to **"Construction"**, select the class, name the ship, and click **"Create Task"**. Repeat for each available slipway.

> **Tip:** Design and research ALL custom components (engines, jump drives, sensors) before creating the ship class. This avoids multiple Research Proto cycles and simplifies the pipeline.

For shipyard retooling details, see [Section 9.1.3 Expanding and Retooling](../9-fleet-management/9.1-shipyards.md). For construction procedures, see [Section 9.2.1 Building Ships](../9-fleet-management/9.2-construction-and-refit.md).
...
5. **Maximum Engine Size 40** (2,000 RP; Power and Propulsion; enables 25+ [HS](../appendices/B-glossary.md#hs) for commercial engines)\hyperlink{ref-7.1-4}{[4]}
6. **Geological Survey Sensors** (1,000 RP; Sensors and Control Systems; the survey sensor itself)\hyperlink{ref-7.1-4}{[4]}
...
- Geological Survey Sensors is in the Sensors and Control Systems category, so it can be researched in parallel with the engine path using a second scientist.
...
\hypertarget{ref-7.1-5}{[5]} Weapon type branching verified against AuroraDB.db DIM\_TechType and FCT\_TechSystem tables. Distinct TechTypeIDs exist for lasers (focal size TechTypeID=15, wavelength TechTypeID=3), particle beams (strength TechTypeID=60, range TechTypeID=61), mesons (focal size TechTypeID=72, focusing TechTypeID=73), plasma carronades (calibre TechTypeID=78), railguns (type TechTypeID=76, velocity TechTypeID=77), gauss cannons (ROF TechTypeID=141, accuracy TechTypeID=143, velocity TechTypeID=144), microwave weapons (focal size TechTypeID=136, focusing TechTypeID=137), and others.

\hypertarget{ref-7.1-6}{[6]} Reactor technology (TechTypeID=41) verified against AuroraDB.db FCT\_TechSystem table. 15 levels from Conventional Reactor (0.5 power, 150 RP) through Quantum Singularity Power Plant (40.0 power, 3,000,000 RP). AdditionalInfo stores the power output multiplier per HS of reactor.
