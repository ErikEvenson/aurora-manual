...
- **Research Bonus:** A percentage bonus (typically ranging from 5% to 50%) applied to research in their specialty field. Higher bonuses are rarer and more valuable.
...
Naval academies produce officers and crew at a rate determined by a training quality slider on the Race screen's Academies tab. The slider ranges from 1 (maximum quantity) to 5 (maximum quality):
...
The bonus only applies when the scientist is researching a technology in their specialty field. Assigning a Power and Propulsion specialist to an Energy Weapons project provides no bonus from their specialization -- and worse, incurs a severe speed penalty.

> **Warning:** Research conducted outside a scientist's specialty field takes approximately **4 times longer** than research in their matched field. This is not merely a loss of bonus -- it is an active penalty that makes out-of-field research dramatically slower. The game does not warn when you queue technologies from the wrong field; you must be aware of this penalty yourself.

- **Primary field:** Full bonus applies and research proceeds at normal speed
- **Secondary field (if the scientist has one):** Reduced bonus applies
- **Unrelated field:** No specialization bonus AND a 4x time penalty (research takes four times as long)

Because of this 4x penalty, it is almost always better to assign a scientist to their matched field even at reduced capacity. A scientist with 25% bonus in their specialty vastly outperforms a 40% bonus scientist working outside their field. Even reassigning a scientist to a new specialization (which cuts their bonus by 75% initially) is preferable to leaving them in the wrong field, because 25% of their original bonus at normal speed still beats full bonus at one-quarter speed.
...
- Labs beyond Admin Rating: Reduced output (approximately 50% of normal per excess lab, though the exact penalty varies)
...
> - Active scientists gain skills slowly over time, potentially improving their research bonus or maximum lab capacity.
> - Changing a scientist's specialization cuts their bonus by 75% initially, but future improvements apply fully to the new field.
...
> - As of v2.0.0, queued project research time estimates include ancient construct bonuses in their calculations.

**Pausing Research (v1.13.0+):**

As of v1.13.0, paused research projects are held at the race level rather than the individual population level. This means pausing a project preserves its progress globally rather than being tied to a specific colony's research infrastructure.
