...
- They have a Build Point cost of **2,400 [BP](../appendices/B-glossary.md#bp)** for a standard lab.\hyperlink{ref-7.3-4}{[4]}
...
The administration rating is a hard cap on the number of labs a scientist can be assigned -- the game UI prevents overallocation (see [Section 7.2.2 Research Bonuses](7.2-scientists.md#722-research-bonuses)) \hyperlink{ref-7.2-6}{[6]}:

- 10 admin scientist with 10 labs: 100% efficiency on all labs
- You cannot assign more than 10 labs to this scientist -- the game enforces the cap
- If you have more labs than one scientist can manage, split the excess to another scientist's project.
...
**Construct Labs Calculation (v2.8.0 clarification):**\hyperlink{ref-7.3-6}{[6]}

Ancient constructs with bonuses below 100% provide usable research labs based on a simple formula:

**Formula:** Bonus / 10 = Usable Labs

| Construct Bonus | Usable Research Labs |
|-----------------|---------------------|
| 100% | 10 labs |
| 70% | 7 labs |
| 50% | 5 labs |
...
For example, an ancient construct with a 70% research bonus in Energy Weapons provides the equivalent of 7 research labs that can be assigned to Energy Weapons research projects at that colony. This is separate from and in addition to any research labs you build on the colony.
...
Ruins vary in technological sophistication based on the extinct civilization's development level (Ruin Race Level 1-6). The maximum research cost for recoverable technology scales with the ruin's tech level:

**Max Research Cost = 2^(TL+1) x 1000 RP**
...
> **v2.8.0+:** Ruins can now occasionally spawn at TL6, allowing recovery of technologies up to 128,000 RP cost. This adds significant variety to ruin exploration, as TL6 sites can yield technologies far beyond what lower-level ruins provide.\hyperlink{ref-7.3-7}{[7]}

For comprehensive coverage of ruin discovery, excavation mechanics, and technology recovery, see [Section 17.3 Xenoarchaeology](../17-exploration/17.3-xenoarchaeology.md).

#### 7.3.4.2.1 Alien Artifact Reserves (v2.8.0)

> **v2.8.0+:** Ruins now leave behind an **Alien Artifact Reserve** after the main ruin items (technologies, installations) are exhausted. This represents minor artifacts overlooked during initial excavation.\hyperlink{ref-7.3-8}{[8]}

**v2.7.1 Behavior (Prior):** In v2.7.1 and earlier, ruins were removed entirely once all recoverable items were found. Xenoarchaeology units assigned to the site became idle with no further recovery possible.

**v2.8.0 Behavior (Current):** The ruin site is removed but an Artifact Reserve persists at the population. Xenoarchaeology units can continue searching for artifacts indefinitely, though with diminishing returns.

**Reserve Characteristics:**

- **Size Range:** Initial reserves range from hundreds to tens of thousands of artifacts, correlated with the original ruin's scope
- **Hidden Value:** The exact reserve size is not displayed to the player; recovery frequency provides the only inference
- **Location:** Reserves are held at the population where the ruin was excavated (displayed in the third column of the population display, below constructs/ruins)\hyperlink{ref-7.3-8}{[8]}

**Recovery Process:**

Xenoarchaeology units assigned to a population with an Artifact Reserve perform a two-step recovery check:\hyperlink{ref-7.3-8}{[8]}

1. **Standard Recovery Check:** Same annual probability as normal ruin excavation (total xenoarchaeology points = annual success percentage)
2. **Reserve Check:** If the standard check succeeds, a secondary check determines artifact recovery with success chance: `Artifact Reserve / 50,000`

**Recovery Yield:** Each successful reserve recovery yields 4D50 (4-200) artifacts.\hyperlink{ref-7.3-8}{[8]}

**Diminishing Success:** Each recovery depletes the reserve, progressively reducing the secondary check's success probability. A reserve of 25,000 has a 50% secondary success chance; after multiple recoveries reduce it to 5,000, the chance drops to 10%.

**Strategic Value:**

The Artifact Reserve mechanic rewards sustained archaeological operations:

- Xenoarchaeology units remain productive after primary excavation completes
- Bodies with large original ruins provide extended artifact income
- Players can maintain excavation infrastructure rather than immediately redeploying units
- Late-game artifact stockpiles accumulate from multiple exhausted sites
...
\hypertarget{ref-7.3-6}{[6]} Aurora Forums — [v2.8.0 Discussion Thread](https://aurora2.pentarch.org/index.php?topic=13894.0) — Ancient construct lab calculation: Bonus / 10 = usable labs (e.g., 70% bonus = 7 labs).

\hypertarget{ref-7.3-7}{[7]} Aurora Forums — [v2.8.0 Changes List](https://aurora2.pentarch.org/index.php?topic=13884.0) — TL6 ruins added in v2.8.0. Maximum tech cost formula: 2^(TL+1) x 1000 RP. TL6 = 128,000 RP max.

\hypertarget{ref-7.3-8}{[8]} Aurora Forums — [v2.8.0 Changes List](https://aurora2.pentarch.org/index.php?topic=13884.0) — Alien Artifact Reserve mechanics: reserves persist after ruin exhaustion, success chance = reserve / 50,000, yield = 4D50 (4-200) artifacts. Artifacts displayed in population third column below constructs/ruins.
