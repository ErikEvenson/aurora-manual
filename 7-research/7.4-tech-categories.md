...
[^1]: Armour progression and costs verified against AuroraDB.db FCT_TechSystem table, v2.7.1.
...
[^3]: Laser wavelength progression verified against AuroraDB.db FCT_TechSystem table. Note: "Near Infrared" does not exist as a separate technology in the database.
...
> **Note:** In the game database, "Logistics" and "Ground Combat" are separate research categories. They are combined here because many players think of them together for strategic planning purposes.
...
Jump point theory (5,000 RP) is the gateway technology for all jump drive capability.[^4] It must be researched before any jump engine can be designed, and it unlocks three sub-technologies that ALL must be completed before a jump drive becomes available in the Ship Designer:

[^4]: Jump Point Theory and sub-technology costs verified against AuroraDB.db FCT_TechSystem table.
...
All three sub-technologies must be researched to at least their first level before the jump engine design options become available. Until then, the jump engine fields in the Ship Designer will show blank entries.

> **Note:** "Max Jump Squadron Size - 3" means 3 ships total can transit together, including the jump-capable ship itself. This is NOT "jump ship + 3 others" -- it is 3 ships total (1 jump ship + 2 escorts).

**Jump Squadron Radius -- Tactical Significance:**

Ships experience disorientation (jump shock) for several minutes after transit and cannot attack or use sensors effectively during this period. The squadron radius determines how spread out a fleet arrives relative to the jump point. A larger radius means ships emerge further from the jump point itself -- this is tactically important because if enemies are defending the jump point, a larger arrival radius reduces the chance of emerging directly on top of hostile forces.
