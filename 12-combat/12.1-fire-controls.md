...
![](../images/art/section-art/12.1.png)

> **[Screenshot Pending — #1081]** Fire Control Configuration Window (CRITICAL for combat mechanics)

Fire controls are the targeting systems that direct weapons to their targets. In Aurora C#, no weapon can fire without an assigned fire control, making them as essential as the weapons themselves. There are two distinct types: beam fire controls ([BFC](../appendices/B-glossary.md#bfc)s) for directing energy weapons and point defense, and missile fire controls ([MFC](../appendices/B-glossary.md#mfc)s) for guiding missile attacks. Each type has different design parameters and tactical considerations.
...
- **Size:** The base BFC size is 1 [HS](../appendices/B-glossary.md#hs), with components scalable up to size 4 for increased range.\hyperlink{ref-12.1-7}{[7]} Single-weapon (SW) variants provide 30-50% reduced size and cost for fighter and FAC applications, with exact values varying based on range and tracking speed selections.\hyperlink{ref-12.1-8}{[8]}
...
![Figure 12.1.1: Beam Fire Control Range Technology Progression](../images/charts/12-combat/12.1.1-beam-fc-range-tech.png)
...
Additional modifiers apply to this base chance (see [Section 12.2.3 Tracking Speed](12.2-beam-weapons.md#1223-tracking-speed) for full hit probability calculation).

**Minimum Tracking Speed (Racial Tracking Speed)**

For hull-mounted (non-turreted) weapons, the effective tracking speed is the *higher* of: \hyperlink{ref-12.1-12}{[12]}

- The ship's maximum speed, OR
- The race's researched Fire Control Speed Rating ("racial tracking speed")

This minimum floor means stationary installations like [PDC](../appendices/B-glossary.md#pdc)s can track targets at the base fire control speed even though the installation itself has 0 km/s speed. A race with Fire Control Speed Rating 10,000 km/s researched will have all hull-mounted weapons track at minimum 10,000 km/s regardless of platform speed.

**Implication:** Reduced Tracking Speed fire control variants (0.5x, 0.75x size/tracking multipliers) provide no practical benefit for hull-mounted weapons, since the racial tracking speed minimum always applies. Only turreted weapons can benefit from tracking speeds exceeding the racial baseline, making turrets essential for engaging fast targets like missiles and fighters.
...
**[ECCM](../appendices/B-glossary.md#eccm) Integration (C# Aurora):**\hyperlink{ref-12.1-4}{[4]}
...
- **[CIWS](../appendices/B-glossary.md#ciws):** ECCM costs 5% of the CIWS fire control component per level, with no size penalty
...
**[ECM](../appendices/B-glossary.md#ecm) Effects on Fire Controls:**\hyperlink{ref-12.1-5}{[5]}
...
MFC range is calculated using the same active sensor range formula (see [Section 11.3.1 Active Sensor Range Formula](../11-sensors-and-detection/11.3-active-sensors.md)):

```
Range (km) = SQRT((Active_Strength x HS x EM_Sensitivity x Resolution^(2/3)) / PI) x 1,000,000
```

*(Note: Earlier versions of this manual contained an incorrect linear formula. The SQRT formula above has been verified against the game database.)*
...
**Resolution Scaling Formula:** \hyperlink{ref-12.1-11}{[11]}

![Fire Control Resolution Scaling](../images/diagrams/12-combat/12.1-resolution-scaling.svg)

When a fire control's resolution exceeds the target's size, effective range is reduced. Because the MFC range formula contains Resolution^(2/3) inside a square root (see [Section A.3.5](../appendices/A-formulas.md#a35-missile-fire-control-range)), the effective range scales as the cube root of the size ratio:

```
Effective Range = Base Range x (Target Size / Resolution Size)^(1/3)
```

*(Note: An earlier version of this section incorrectly stated the scaling was quadratic, citing a forum post attributed to the developer. The cube-root formula above is verified against the game database — substituting target size for resolution in the active sensor range formula produces ranges matching DB values within rounding error across all tested components.)* \hyperlink{ref-12.1-13}{[13]}

This cube-root relationship means resolution mismatches are significant but not as extreme as a quadratic model would suggest:

| Fire Control Resolution | Target Size | Effective Range |
|------------------------|-------------|-----------------|
| 160 (8,000 tons) | 8,000 tons | 100% of base range |
| 160 (8,000 tons) | 4,000 tons | ~79.4% of base range |
| 160 (8,000 tons) | 2,000 tons | ~63.0% of base range |
| 160 (8,000 tons) | 750 tons | ~45.4% of base range |

**Example:** A Resolution-160 fire control with 100,000 km base range against a 750-ton FAC:
- Effective Range = 100,000 x (15/160)^(1/3) = 100,000 x 0.454 = **45,428 km**
- Range is reduced to under half, but the fire control retains meaningful engagement distance

While the penalty is less extreme than a quadratic model, dedicated low-resolution fire controls still provide substantially better range against small targets. A Resolution-20 fire control would detect the same 750-ton FAC at (15/20)^(1/3) = 90.9% of base range, compared to 45.4% for the Resolution-160 unit.
...
\hypertarget{ref-12.1-11}{[11]}. Aurora Forums -- Steve Walmsley response in "Struggling with targeting and launching missiles" (https://aurora2.pentarch.org/index.php?topic=13714.0). Originally cited as confirming a quadratic scaling formula; however, database verification (see [13]) demonstrates cube-root scaling. The forum post may have been misinterpreted or the formula may have changed in a subsequent version.

\hypertarget{ref-12.1-12}{[12]}. AuroraWiki -- Combat Overview (https://aurorawiki2.pentarch.org/index.php?title=Combat_Overview). "Hull mounted weapons have a tracking speed of either the ship's max speed or the current base beam fire control tech, whichever is greater."

\hypertarget{ref-12.1-13}{[13]}. Aurora C# game database (AuroraDB.db v2.7.1) -- Resolution scaling verified as cube-root: Effective Range = Base Range x (Target/Resolution)^(1/3). Derived from the active sensor range formula containing Resolution^(2/3) under SQRT. Tested against FCT_ShipDesignComponents for 6-HS active sensors (SDComponentIDs 153671, 153674, 153675): R-102 range 73,053,084 km -> R-20 predicted 42,440,631 km (actual 42,440,631 km); R-102 -> R-5 predicted 26,735,922 km (actual 26,735,922 km). Zero error across all test cases.
