...
- **Size:** The base BFC size is 1 HS, with components scalable up to size 4 for increased range.\hyperlink{ref-12.1-7}{[7]} Single-weapon (SW) variants provide reduced size and cost for fighter and FAC applications, with sizes varying based on range and tracking speed selections.
...
A specialized fire control variant designed for dedicated weapon systems. Only a single beam weapon can be assigned to a fire control designed for single weapon use. The size, cost, and crew requirements of single weapon fire controls are significantly reduced compared to multi-weapon fire controls (typically 30-50% reduction depending on range and tracking speed settings).\hyperlink{ref-12.1-8}{[8]} *(unverified — #728 -- version-specific feature requires live testing to confirm v1.13+ availability)*
...
1. Check the "Assign #" checkbox in the Ship Combat tab of the Naval Organization window (as of v1.12.0, fire controls on this tab are sorted by range for easier management) *(unverified — #728 -- version-specific UI change)*
...
All weapons have a number suffix (e.g., "10cm Railgun #1", "10cm Railgun #2"). When you drag weapon #5 with "Assign # 4", weapons #6, #7, and #8 also transfer. If higher-numbered weapons are already assigned elsewhere, the system skips them and takes the next available. Lower-numbered weapons never transfer; only ascending sequences from the selected weapon move. *(unverified — #728 -- UI mechanic)*
...
- **Open Fire:** Fires at any valid target within range. Beam weapons will not fire if they have zero chance to hit, preventing wasted fire. As of v1.12.0, the auto-fire system checks the actual weapon range rather than the fire control range when determining engagement eligibility, preventing weapons from being assigned targets beyond their effective reach. *(unverified — #728 -- version-specific change)*
- **Point Blank Defensive Fire:** Engages hostile missiles within 10,000 km during movement phase; always calculates hit chance at 10,000 km range baseline *(unverified — #728)*
...
Fire delay occurs when inexperienced ships open fire or change targets. The formula is: *(unverified — #728 -- forum-sourced formula)*
...
- The delay is approximately half that of VB6 and follows a bell curve distribution, making maximum delays far less probable *(unverified — #728)*
- Fire delays are applied in three situations: jump point transit, opening fire, and changing target *(unverified — #728)*
...
Fire delay will only apply to point defence fire if it is caused by jump point transit. This prevents a single offensive fire control changing targets from imposing delays across all ship fire controls, including those dedicated to point defence. *(unverified — #728 -- version-specific change)*
...
1. **Ship CIWS** engages first (as of v1.12.0, CIWS always targets the closest incoming missile regardless of salvo ordering, improving point-blank defense effectiveness) *(unverified — #728 -- version-specific change)*
...
A Combat Comparison tab in the Naval Organization window displays a comprehensive list of all ships with combat history. Features include: *(unverified — #728 -- version-specific feature)*
