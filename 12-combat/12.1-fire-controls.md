...
- **Size:** BFC size is fixed at 1 HS (50 tons) for standard beam fire controls. Smaller versions (0.5 HS) are available at reduced range and tracking speed for use on fighters and FACs. *(unverified)*
...
1. Check the "Assign #" checkbox in the Ship Combat tab of the Naval Organization window (as of v1.12.0, fire controls on this tab are sorted by range for easier management) *(unverified -- version-specific UI change)*
...
All weapons have a number suffix (e.g., "10cm Railgun #1", "10cm Railgun #2"). When you drag weapon #5 with "Assign # 4", weapons #6, #7, and #8 also transfer. If higher-numbered weapons are already assigned elsewhere, the system skips them and takes the next available. Lower-numbered weapons never transfer; only ascending sequences from the selected weapon move. *(unverified -- UI mechanic)*
...
Fire delay occurs when inexperienced ships open fire or change targets. The formula is: *(unverified -- forum-sourced formula)*
...
- Ships with 100% Fleet Training (500 points) experience zero fire delay *(unverified)*
- The delay is approximately half that of VB6 and follows a bell curve distribution, making maximum delays far less probable *(unverified)*
- Fire delays are applied in three situations: jump point transit, opening fire, and changing target *(unverified)*
...
Fire delay will only apply to point defence fire if it is caused by jump point transit. This prevents a single offensive fire control changing targets from imposing delays across all ship fire controls, including those dedicated to point defence. *(unverified -- version-specific change)*
...
Any beam fire control with a tracking speed at least 2x the racial base speed is automatically classified as a point defence fire control by the game's weapon assignment system. *(unverified)*
...
1. **Ship CIWS** engages first (as of v1.12.0, CIWS always targets the closest incoming missile regardless of salvo ordering, improving point-blank defense effectiveness) *(unverified -- version-specific change)*
...
A Combat Comparison tab in the Naval Organization window displays a comprehensive list of all ships with combat history. Features include: *(unverified -- version-specific feature)*
...
\hypertarget{ref-12.1-4}{[4]}. Aurora Wiki (ECM, ECCM articles) and official forum sources -- ECCM integration costs: 10% for BFC/MFC, 5% for CIWS (no size penalty), 10% for STO.

\hypertarget{ref-12.1-5}{[5]}. Aurora Wiki (ECM, ECCM articles) -- ECM reduces MFC lock-on range by 20% per net ECM level (Sensor Jammer advantage). Against beam FCs, Fire Control Jammers reduce hit probability by 20% per net advantage level.
