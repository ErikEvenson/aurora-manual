...
Note: The above table shows approximate base values assuming Infrared wavelength (lowest range multiplier) and Capacitor Recharge Rate 1 (base recharge). These values are illustrative starting points only. Exact size, damage, and range depend on Laser Focal Size, Laser Wavelength, and Capacitor Recharge Rate technologies researched.\hyperlink{ref-12.2-1}{[1]} Higher-frequency wavelength tech (UV, X-Ray, etc.) increases range; higher capacitor tech increases size and power requirements. See [Section 12.2.7 Beam Weapon Reference Tables](#1227-beam-weapon-reference-tables) for verified reference examples at specific tech levels (e.g., 10cm C3 UV = 3 HS, 120,000 km range; 20cm C5 UV = 6 HS, 400,000 km range). *(unverified — #732 -- approximate table values not individually verified against AuroraDB)*
...
- Must be built into the ship design from the start (cannot be refitted) *(unverified — #732 -- requires live testing)*
- The ship can only fire the spinal mount at targets within its forward arc *(unverified — #732 -- requires live testing)*
...
For lasers and other weapons with a damage gradient greater than 1, damage decreases linearly with range: *(unverified — #732 -- requires live testing to confirm formula)*
...
The damage gradient value is a separate concept from range falloff. It determines how damage is distributed across the target's armor columns on each hit: *(unverified — #732 -- requires live testing to confirm gradient values)*
...
- **Crew Training:** Training grade from 0.1 (untrained) to 1.0 (fully trained), typically affecting overall combat effectiveness *(unverified — #732 -- requires live testing to confirm range)*
...
When a ship's power plants are damaged and cannot supply full power to all weapons simultaneously, power is allocated weapon by weapon until the available power is exhausted. This means some weapons may not be recharged at all, but those that do receive power are recharged at the maximum rate. *(unverified — #732 -- requires live testing to confirm C# mechanic)*
...
**Research Category:** Gauss cannon technologies are researched under the **Gauss Cannon** category (CategoryID 39 in the database), which is separate from both Energy Weapons and Missiles. \hyperlink{ref-12.2-16}{[16]}
...
**In C# Aurora, there is no penalty for energy weapons firing in or through an atmosphere.** This is a significant change from VB6 Aurora, which imposed restrictions on energy weapon effectiveness in atmospheric conditions. *(unverified — #732 -- requires live testing or changelog verification)*
