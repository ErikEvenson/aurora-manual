...
**Damage Falloff with Range (Damage Gradient)**

Beam weapon damage falloff is controlled by the weapon's **damage gradient** value. Most lasers use a higher gradient (typically 3 or 4), meaning damage is spread across multiple range increments and decreases with distance. Some weapons (particle beams, plasma carronades, railguns) have a damage gradient of 1, meaning they deal full damage at any range within their maximum.

For weapons with damage falloff, the damage is divided into discrete range brackets. A weapon with maximum range of 320,000 km and a standard gradient has its damage distributed across range increments (typically in 10,000 km steps). At each bracket, the weapon does proportionally less damage:
...
**Damage Gradient Values**

The damage gradient determines how damage is distributed across the weapon's armor impact columns:

- **Gradient 1** (Missiles, Plasma Carronades, Particle Beams, Railguns): Full damage at all ranges; damage concentrated in a single armor column
- **Gradient 2**: Moderate damage spread across armor columns
- **Gradient 3** (Standard Lasers): Damage spread across 3 armor columns; reduces with range
- **Gradient 4** (Some large lasers): Damage spread across 4 armor columns; reduces with range

Higher gradient values spread damage across more armor columns, making each individual hit less likely to penetrate deep armor but more likely to wear down a wider area.
