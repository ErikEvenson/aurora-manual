...
Note: The above table shows approximate base values at starting technology. Exact size, damage, and range depend on Laser Focal Size, Laser Wavelength, and Capacitor Recharge Rate technologies researched. Higher wavelength tech increases range; higher capacitor tech increases size and power requirements. See [Section 12.2.7 Beam Weapon Reference Tables](#1227-beam-weapon-reference-tables) for ground-truth reference examples at specific tech levels (e.g., 10cm C3 UV = 3 HS, 120,000 km range; 20cm C5 UV = 6 HS, 400,000 km range).
...
For lasers and other weapons with a damage gradient greater than 1, damage decreases linearly with range:
...
The damage gradient value is a separate concept from range falloff. It determines how damage is distributed across the target's armor columns on each hit:
...
| Level 10 | 400 seconds | 80% | 480,000 RP |[^tracking-time]

[^tracking-time]: Tracking time bonus technology progression verified against AuroraDB.db (TechSystemID 27653-27658, 74206-74210).
...
At the point when any weapon (energy-based or missile launcher) fires, there is a **2% chance the weapon will suffer a failure**.[^weapon-failure]

[^weapon-failure]: Weapon failure rate verified via Aurora Wiki (C-Ship-Combat) and official forum sources.
...
**Research Category:** As of v2.0.0, gauss cannon technologies (including gauss turrets) are researched under the **Missiles and Kinetic Weapons** category rather than the Energy Weapons category. Additionally, 10cm railguns are available as a point defense option for NPR designs.
...
| 5 shots | 45,000 RP |
| 6 shots | 135,000 RP |
| 8 shots | 750,000 RP |[^gauss-rof]

[^gauss-rof]: Gauss Cannon Rate of Fire research costs verified against AuroraDB.db (TechSystemID 26612, 26630).
