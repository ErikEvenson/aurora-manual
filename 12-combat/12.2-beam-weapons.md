...
- Must be built into the ship design from the start (cannot be refitted) *(unverified — #732)*
- The ship can only fire the spinal mount at targets within its forward arc *(unverified — #732)*
...
For lasers and other weapons with a damage gradient greater than 1, damage decreases linearly with range: *(unverified — #732 -- formula sourced from Aurora Wiki/forums)*
...
The damage gradient value is a separate concept from range falloff. It determines how damage is distributed across the target's armor columns on each hit: *(unverified — #732 -- specific gradient values sourced from Aurora Wiki)*
...
- **Crew Training:** Training grade from 0.1 (untrained) to 1.0 (fully trained), typically affecting overall combat effectiveness *(unverified — #732 -- specific range values)*
...
When a ship's power plants are damaged and cannot supply full power to all weapons simultaneously, power is allocated weapon by weapon until the available power is exhausted. This means some weapons may not be recharged at all, but those that do receive power are recharged at the maximum rate. *(unverified — #732 -- C# mechanic)*
...
**Gauss Cannon Reference Example:** *(unverified — #732 -- component stat block from in-game design)*
...
- Size: 7 HS (350 tons) for standard particle beams *(unverified — #732 -- specific size claim)*
...
**Particle Beam Reference Example:** *(unverified — #732 -- component stat block from in-game design)*
...
**Plasma Carronade Reference Example:** *(unverified — #732 -- component stat block from in-game design)*
...
*(unverified — #732 -- component stat blocks from in-game design)*
...
**In C# Aurora, there is no penalty for energy weapons firing in or through an atmosphere.** This is a significant change from VB6 Aurora, which imposed restrictions on energy weapon effectiveness in atmospheric conditions. *(unverified — #732 -- version-specific change)*
