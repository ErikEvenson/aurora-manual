...
- **Black Holes** - Stellar-mass black holes ranging from 1 to 120 solar masses across fifteen types \hyperlink{ref-4.1-5}{[5]}. Key characteristics:
  - Always planetless and do not affect ship movement *(unverified)*
  - **Random Stars games:** 1.5% chance of generation *(unverified)*; may have companion stars *(unverified)*
  - **Known Stars games:** Eighty random black holes are added to the Known Stars list *(unverified)* using random XYZ coordinates within plus or minus one hundred light years from Sol, representing undetected objects; these lack companion stars *(unverified)*
...
Aurora C# uses a hierarchical system for naming newly discovered star systems *(unverified)*. When a system is first discovered, names are assigned using this priority sequence *(unverified)*:
...
Players can optionally assign one of the available naming themes to any system \hyperlink{ref-4.1-19}{[19]}. This choice persists, so all future exploration beyond that point will use the selected naming theme *(unverified)*. This enables different jump point chains to have distinct naming conventions, helping players organize their galactic nomenclature by region or faction preference.

Players retain the ability to manually rename systems at any time and can select names from any available naming theme or use custom text *(unverified)*.
...
Known Stars games include an optional feature to add a hypothetical distant planet to the Sol system *(unverified)*, inspired by real scientific theories about undiscovered bodies affecting Kuiper Belt object orbits.
...
The SpaceMaster can trigger a supernova at any star system, regardless of scientific accuracy *(unverified)*. This feature is restricted to Known Stars games because it requires mapped 3D locations in real space *(unverified)*. When triggered, all jump points, system bodies, and military forces in the affected system are destroyed *(unverified)*.

### Radiation Wave Propagation

The supernova creates an expanding spherical radiation front that weakens as surface area increases *(unverified)*. When the wave reaches a new star system, it irradiates every body in that system and may damage ships and missiles *(unverified)*.
...
- Real space distance in light-years between systems *(unverified)*
- Time to supernova impact for the nearest radiation wave *(unverified)*
- Projected radiation levels upon impact *(unverified)*
...
\hypertarget{ref-4.1-18}{[18]}. AuroraWiki, "System Map" (aurorawiki.pentarch.org/index.php?title=System_Map) -- System Map displays stars, orbital paths, planets, jump points, ships, and colony markers in a top-down orbital view.
