...
![](../images/art/section-art/18.1.png)
...
- At maximum range, hit chance approaches 0% per the formula in [Section 18.1.3.2](#18132-beam-weapon-to-hit-calculation): Base\_Hit\_Chance = (Max\_Range - Max\_Range) / Max\_Range = 0% \hyperlink{ref-18.1-1}{[1]}
...
![Figure 18.1.2: Production Formula Breakdown](../images/diagrams/18-advanced-topics/18.1-production-formula.svg)

![Production Formula Detailed Breakdown](../images/diagrams/18-advanced-topics/18.1-production-formula-breakdown.png)
...
- **Sector_Bonus** = Governor's Production bonus x 0.25 for Industrial command type administrators \hyperlink{ref-18.1-4}{[4]}
...
![Figure 18.1.3.1: Combat Sub-Pulse Sequence](../images/diagrams/18-advanced-topics/18.1-combat-turn-sequence.svg)

![Combat Sub-Pulse Sequence Detailed](../images/diagrams/18-advanced-topics/18.1-combat-subpulse-sequence.png)
...
![Figure 18.1.3.2: Beam Weapon To-Hit Calculation Flow](../images/diagrams/18-advanced-topics/18.1-beam-tohit-calculation.svg)

![Beam Weapon To-Hit Calculation Detailed](../images/diagrams/18-advanced-topics/18.1-beam-tohit-flow.png)
...
> *[v2.2.0]: Missile agility was removed from hit calculations. Hit chance is now based entirely on speed ratio plus optional Active Terminal Guidance.*

**Active Terminal Guidance** (0.25 MSP component, costs Uridium) provides an additive accuracy bonus from +15% to +60% based on technology level. See [Section 12.3 Missiles](../12-combat/12.3-missiles.md) for the full technology progression.
...
Weapons that bypass armor (such as meson cannons) strike internal components directly. Meson cannons are single-target weapons, not area-effect; they ignore armor on the targeted ship but do not deal splash damage. This makes them extremely dangerous against individual targets but they typically have shorter range and lower accuracy.

Missile warheads can be designed with variable damage ratings. Higher-damage warheads destroy more armor columns per hit but increase missile size, reducing speed and making them easier to intercept. Note: missile agility was removed from hit calculations in v2.2.0 (see [Section 18.1.3.3](#18133-missile-hit-resolution)); hit chance is now based entirely on the speed ratio between missile and target plus optional Active Terminal Guidance.
...
- The code makes 20 attempts to find a new system before accepting a link to an existing system *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837) -- requires live testing)*
- Any newly generated system created via the final unlinked jump point must contain a minimum of two jump points *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837) -- requires live testing)*
...
- Hit chance is reduced to 33% of normal *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837) -- requires live testing)*
...
- Hit chance is 50% of normal (higher than Search and Destroy since targets are pre-identified) *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837) -- requires live testing)*
...
\hypertarget{ref-18.1-1}{[1]}. Aurora C# beam combat to-hit formula: Base\_Hit\_Chance = (Max\_Range - Current\_Range) / Max\_Range x 100. At maximum range, this yields 0%. The previous claim of "50% at max range" was incorrect and contradicted the formula in Section 18.1.3.2. Corrected per formula verification. -- AuroraWiki, "Combat Overview" (aurorawiki2.pentarch.org)
...
\hypertarget{ref-18.1-4}{[4]}. Aurora C# game database (AuroraDB.db v2.7.1) -- DIM\_NavalAdminCommandType CommandTypeID=6 (Industrial): Industrial column=0.25, meaning the Industrial command type passes 25% of the commander's Production bonus to subordinate populations. Note: "Sector Command" is a separate planetary installation (DIM\_PlanetaryInstallation ID=20), not a naval admin command type. Other command types pass different fractions of different bonuses (e.g., Naval passes 0.25 of Engineering, CrewTraining, Tactical, and Reaction but 0.0 of Industrial).
