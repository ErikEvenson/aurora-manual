...
The base fuel production rate is 20,000 litres per refinery per year \hyperlink{ref-18.1-8}{[8]}, upgradeable through fuel production technology. Refineries consume Sorium at a fixed ratio of 1 ton per 2,000 litres refined. Each refinery requires a matching supply of Sorium from mining or stockpiles.
...
- The code makes 20 attempts to find a new system before accepting a link to an existing system *(unverified -- internal code behavior not publicly documented)*
- Any newly generated system created via the final unlinked jump point must contain a minimum of two jump points *(unverified -- internal code behavior not publicly documented)*
...
- Hit chance is reduced to 33% of normal *(unverified)*
...
- Hit chance is 50% of normal (higher than Search and Destroy since targets are pre-identified) *(unverified)*
...
\hypertarget{ref-18.1-1}{[1]}. Aurora C# beam combat to-hit formula: at maximum range, the roll must be less than or equal to the range bracket value. Since the range bracket at max range is roughly half of max range, approximately 50% of shots hit. -- AuroraWiki, "Combat Overview" (aurorawiki.pentarch.org)
...
\hypertarget{ref-18.1-6}{[6]}. Aurora C# standard mineral deposit mechanic -- Accessibility values range from 0.1 to 1.0 as documented on AuroraWiki, "Population and Production" (aurorawiki.pentarch.org)
...
\hypertarget{ref-18.1-8}{[8]}. AuroraWiki, "Installations" -- Base fuel refinery production rate is 20,000 litres/year per refinery (not 2,000 as previously stated). Sorium conversion ratio is 1 ton per 2,000 litres refined. **Correction:** The original text stated "2000\_litres"; the actual base rate is 20,000 litres/year, upgradeable via Fuel Production technology (DB TechTypeID=32, starting at 48,000 litres).
...
\hypertarget{ref-18.1-12}{[12]}. Aurora C# standard combat resolution occurs in 5-second sub-pulses during active combat -- AuroraWiki, "Combat Overview" (aurorawiki.pentarch.org)
...
\hypertarget{ref-18.1-15}{[15]}. Aurora C# missile hit chance formula -- AuroraWiki, "Combat Overview" (aurorawiki.pentarch.org)

\hypertarget{ref-18.1-16}{[16]}. Aurora C# internal damage resolution -- each component has HTK values; the database tracks component HTK in FCT\_ShipDesignComponents. AuroraWiki, "Combat Overview" (aurorawiki.pentarch.org)
