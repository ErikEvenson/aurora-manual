...
- **Combat**: To-hit rolls, beam damage variance, missile evasion checks
...
The base construction rate is determined by the number of Construction Factories (or Conventional Industry) and their per-unit output:
...
Combat in Aurora C# is resolved in a specific sequence within each combat sub-pulse (typically 5-second intervals during active engagements):
...
When a wreck is salvaged, recovered minerals and components go to freighters in the same fleet as the salvage ship. C# Aurora introduces enhanced mechanics for orbital salvage operations.
...
Fighters can be assigned non-support missions when operating over battlefields. These require a fleet composed only of fighters with orders targeting a system body as destination.
