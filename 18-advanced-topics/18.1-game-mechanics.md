...
- **Combat** (see [Section 12 Combat](../12-combat/12.1-combat-overview.md)): To-hit rolls, beam damage variance, missile evasion checks
...
The base construction rate is determined by the number of Construction Factories (see [Section 6 Economy and Industry](../6-economy-and-industry/6.2-installations.md)) (or Conventional Industry) and their per-unit output:
...
Combat in Aurora C# is resolved in a specific sequence within each combat sub-pulse (see [Section 12 Combat](../12-combat/12.1-combat-overview.md)) (typically 5-second intervals during active engagements):
...
When a wreck is salvaged, recovered minerals and components go to freighters in the same fleet (see [Section 9 Fleet Management](../9-fleet-management/9.1-fleet-organization.md)) as the salvage ship. C# Aurora introduces enhanced mechanics for orbital salvage operations.

**Wreck Size Precision (v2.0+):** As of v2.0, wreck sizes are no longer rounded down to the nearest 50 tons. Wrecks now retain their exact tonnage, providing more accurate salvage yields and better intelligence about the original ship's displacement.
...
Fighters (see [Section 12 Combat](../12-combat/12.3-fighters.md)) can be assigned non-support missions when operating over battlefields. These require a fleet composed only of fighters with orders targeting a system body as destination.
...
- [Section 6 Economy and Industry](../6-economy-and-industry/6.2-installations.md) -- Production formulas and construction factory output
- [Section 8 Ship Design](../8-ship-design/8.1-design-overview.md) -- Ship design parameters used in formula calculations
- [Section 12 Combat](../12-combat/12.1-combat-overview.md) -- Combat resolution mechanics and weapon to-hit formulas
- [Section 13 Ground Forces](../13-ground-forces/13.1-ground-unit-design.md) -- Fighter non-support missions against ground targets
