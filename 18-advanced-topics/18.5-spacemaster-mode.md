...
While Aurora C# always starts the player in Sol, SpaceMaster mode can be used after game creation to set up a custom starting location. This involves creating a new system, establishing a colony with installations, and optionally ignoring the default Sol-based empire. See [Section 2.5.3.2 Custom Start via SpaceMaster](../2-game-setup/2.5-starting-conditions.md#2532-custom-start-via-spacemaster-advanced) for details.

> **Note:** There is no built-in "random start" option — custom starting locations require manual SpaceMaster setup. You cannot select a specific real star (like Alpha Centauri) as your starting location.
...
| Custom Start | Manually set up a non-Sol starting location (advanced) |
