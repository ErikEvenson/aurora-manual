...
![18.5 SpaceMaster Mode](../images/art/section-art/18.5.png)
...
SpaceMaster mode provides tools for creating and modifying star systems \hyperlink{ref-18.5-4}{[4]}. Access these by selecting **"Open System View"** from the menu bar while SM mode is active. Buttons appear in the lower right corner:

- **Create System** — Generates a new random star system. With Known Stars enabled, you can select from nearby real stars (G or K-type recommended for habitability).
- **Delete System** — Removes an unwanted system, allowing regeneration.
- **Modify Body** — Edits planetary characteristics including distance from star, diameter, hydro extent (water coverage), albedo, density, and day length.
- **Atmosphere Panel** — Add or remove atmospheric gases (nitrogen, oxygen, etc.) and monitor resulting temperature changes.

These tools enable creating custom starting scenarios. See [Section 2.5.3.2 Custom Start via SpaceMaster](../2-game-setup/2.5-starting-conditions.md#2532-custom-start-via-spacemaster-advanced) for a complete walkthrough.
...
- The Instant Organisation feature for ground forces consumes build points when used in SM mode as of v2.3.1 *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837) -- requires live testing)* (see [Section 13.1 Unit Types](../13-ground-forces/13.1-unit-types.md))
...
- Restore deceased or retired officers to active duty (SM Mode Only; see [Section 16.1 Officer Generation](../16-commanders/16.1-officer-generation.md)) *(unverified — [#837](https://github.com/ErikEvenson/aurora-manual/issues/837) -- requires live testing)*
...
\hypertarget{ref-18.5-1}{[1]}. Aurora C# game database (AuroraDB.db v2.7.1) -- FCT\_Game: SMPassword field (TEXT, default=NULL) stores the SpaceMaster mode password, confirming password protection is a database-level game feature.
...
\hypertarget{ref-18.5-3}{[3]}. Aurora C# game database (AuroraDB.db v2.7.1) -- FCT\_ShipDesignComponents table tracks all 11 mineral costs (Duranium through Gallicite columns), confirming the complete mineral list matches what SM mode can manipulate.

\hypertarget{ref-18.5-4}{[4]}. Aurora Forums, "How to Generate a Custom Starting System or Species" — Tutorial describing SpaceMaster system creation tools: Create System, Delete System, Modify Body, Create Race. Access via "Open System View" menu with SM mode active. [aurora2.pentarch.org](https://aurora2.pentarch.org/index.php?topic=11136.0)

**Note:** SpaceMaster mode functionality is primarily documented through AuroraWiki articles and Steve Walmsley's forum posts on aurora2.pentarch.org. SM tools appear in the lower right corner of the System View when SM mode is active.
