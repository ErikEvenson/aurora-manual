...
Seven command and control modules establish a ship's Control Rating (each module adds +1 to the rating):
...
When multiple ships operate together in a task group, a Task Group Commander provides fleet-level coordination. The task group commander is automatically the senior officer among all ship commanders in the group, or can be specifically assigned as a flag officer aboard a designated flagship.
...
- **Mining Bonus**: Improves the output of all mining operations (both manual mines and automated mines) on the colony. A governor with 30% Mining skill increases mineral extraction by 30%.
...
- **Assignment**: Each research project requires an assigned scientist. Open the Research window, select a project, and assign a scientist from the available pool.
...
- **Fleet Train**: Officers commanding logistics vessels (tankers, supply ships) with high Logistics skill reduce fuel waste and improve supply delivery rates.
