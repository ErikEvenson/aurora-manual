...
When commanders are generated, their bonuses are determined randomly according to probability bands. The following table shows the chance each bonus type is generated for new officers *(unverified)*:
...
A ship needs either a commander or executive officer with a crew training bonus to conduct training *(unverified)*. Each officer's contribution is calculated separately, then combined.
...
The final bonus is halved for the ship commander. The executive officer uses the full bonus *(unverified)*. This asymmetry means executive officers provide greater training benefits.

**Points Accumulation:**

> Incremental Crew Grade Points per Year = Final Bonus x 500 *(unverified)*

Maximum crew grade: 1000 points *(unverified)*.

**Example:** An executive officer with a 20% bonus supported by a naval admin command (30% bonus) would generate approximately 145 crew grade points annually. *(unverified)*

### Grade Bonus Formula

The crew grade bonus is calculated as:

> Grade Bonus = SQRT(Grade Points) - 10 *(unverified)*

This produces a maximum possible bonus of approximately 21.6% in C# Aurora (assuming 1000 maximum grade points: SQRT(1000) - 10 = 21.6). *(unverified)*
...
> Incremental Fleet Training Points per Year = Final Bonus x 1000 *(unverified)*
...
> Morale = Intended Deployment Time / Deployment Clock *(unverified)*
...
> Morale Modifier = (Current Crew / Class Crew) x 2 *(unverified)*

**Morale Floor:** Morale cannot drop below 25% under normal circumstances. *(unverified)*

**Life Support Failure:** When life support systems are destroyed, morale is immediately reduced to 10%. *(unverified)*

**Deployment Clock Reduction:**

- Reduces at 10x the normal rate when at recreational locations *(unverified)*
- Recreational locations include ships with recreational modules or populations exceeding 50,000 *(unverified)*
...
Some bonuses may experience diminishing returns at very high values. An officer with 80% in a skill does not necessarily provide twice the benefit of one with 40%. The exact diminishing returns curve varies by skill type, but in general *(unverified)*:
...
The standard rank progression for naval officers is *(unverified)*:
