...
When commanders are generated, their bonuses are determined randomly according to probability bands. The following table shows the chance each bonus type is generated for new officers *(unverified — #713)*:
...
A ship needs either a commander or executive officer with a crew training bonus to conduct training *(unverified — #713)*. Each officer's contribution is calculated separately, then combined.
...
The final bonus is halved for the ship commander. The executive officer uses the full bonus *(unverified — #713)*. This asymmetry means executive officers provide greater training benefits.

**Points Accumulation:**

> Incremental Crew Grade Points per Year = Final Bonus x 500 *(unverified — #713)*

Maximum crew grade: 1000 points *(unverified — #713)*.

**Example:** An executive officer with a 20% bonus supported by a naval admin command (30% bonus) would generate approximately 145 crew grade points annually. *(unverified — #713)*

### 16.2.3.2 Grade Bonus Formula

The crew grade bonus is calculated as:

> Grade Bonus = SQRT(Grade Points) - 10 *(unverified — #713)*

This produces a maximum possible bonus of approximately 21.6% in C# Aurora (assuming 1000 maximum grade points: SQRT(1000) - 10 = 21.6). *(unverified — #713)*
...
> Incremental Fleet Training Points per Year = Final Bonus x 1000 *(unverified — #713)*
...
> Morale = Intended Deployment Time / Deployment Clock *(unverified — #713)*
...
> Morale Modifier = (Current Crew / Class Crew) x 2 *(unverified — #713)*

**Morale Floor:** Morale cannot drop below 25% under normal circumstances. *(unverified — #713)*

**Life Support Failure:** When life support systems are destroyed, morale is immediately reduced to 10%. *(unverified — #713)*

**Deployment Clock Reduction:**

- Reduces at 10x the normal rate when at recreational locations *(unverified — #713)*
- Recreational locations include ships with recreational modules or populations exceeding 50,000 *(unverified — #713)*
...
Some bonuses may experience diminishing returns at very high values. An officer with 80% in a skill does not necessarily provide twice the benefit of one with 40%. The exact diminishing returns curve varies by skill type, but in general *(unverified — #713)*:
...
The standard rank progression for naval officers is *(unverified — #713)*:
