...
![](../images/art/section-art/1.1.png)
...
- Dramatically improved performance, especially for large games with many [NPR](../appendices/B-glossary.md#npr) (Non-Player Races) \hyperlink{ref-1.1-8}{[8]}
...
> **Important Notice**: This manual was initially generated using AI assistance (Claude) and has undergone systematic verification against authoritative sources. All chapters and appendices have been verified, with numbered references added throughout and factual corrections applied where needed. Claims that could not be verified against authoritative sources are individually marked with *(unverified — [#NNN])* linking to tracking issues.

**Version Coverage**:

This manual covers Aurora C# v2.7.1 (current stable) with v2.8.0 changes documented where applicable. Version-specific differences are marked using:

- Blockquote callouts: `> **v2.8.0+:**` for significant behavior changes
- Inline markers: `*(v2.7.1)* / *(v2.8.0+)*` for value differences

When no version marker appears, content applies to both versions.

**Verification Methodology**:

Formulas and values were verified against the Aurora C# game database (AuroraDB.db v2.7.1), Steve Walmsley's forum posts, and the AuroraWiki. Verified claims include numbered references (e.g., [1]) linking to their sources. Claims marked *(unverified)* could not be confirmed from available sources and should be treated as community knowledge or approximations. Version 2.8.0 changes were sourced from the [Aurora Forums v2.8.0 Changes List](https://aurora2.pentarch.org/index.php?topic=13884.0).
