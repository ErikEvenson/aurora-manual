...
Jump points are gravitational anomalies in space where the fabric of spacetime is thin enough to allow transit between star systems. Each jump point connects exactly two systems, creating a network of interstellar pathways. *(unverified)* Key properties:
...
- **Guaranteed Exploration Path (v2.1.1)**: The jump point generation system prevents "closed universe" situations by ensuring at least one unexplored avenue always exists. Systems generated from the last unlinked jump point are guaranteed to contain a minimum of two jump points, ensuring exploration can always continue. *(unverified -- v2.1.1 guarantee not confirmed in database)*
...
- **Stellar Lagrange Points**: Points defined by the primary star's interaction with the galactic center of mass. These are the most common jump point locations. *(unverified)*
- **Planetary Lagrange Points**: Points created by large planets' gravitational influence. Less common but possible. *(unverified)*
- **Binary Star Points**: In binary or multiple star systems, the interaction between stellar components creates additional potential jump point locations. *(unverified)*
...
- System star types (certain star types are more likely to have habitable worlds). *(unverified)*
...
Gravitational surveys can also discover anomalies at survey locations. In v1.10.0+, these anomalies may contain Ancient Constructs -- structures left behind by a precursor civilization that provide research bonuses and serve as defenses against Aether Rifts. *(unverified -- v1.10.0 anomaly/Ancient Construct mechanics not confirmed in database)*
