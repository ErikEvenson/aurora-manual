...
Jump points are gravitational anomalies in space where the fabric of spacetime is thin enough to allow transit between star systems. Each jump point connects exactly two systems, creating a network of interstellar pathways. Key properties:

- **Bidirectional**: A jump point (see [Section 10 Navigation](../10-navigation/10.1-jump-points.md)) always has a corresponding exit point in the destination system. Transiting in either direction is possible.
- **Fixed Position**: Jump points do not move. Once discovered, their position in the system is permanent.
- **Variable Number**: Each system has a random number of jump points, typically between 2 and 8, though extremes in either direction are possible.
- **Guaranteed Exploration Path (v2.1.1)**: The jump point generation system prevents "closed universe" situations by ensuring at least one unexplored avenue always exists. Systems generated from the last unlinked jump point are guaranteed to contain a minimum of two jump points, ensuring exploration can always continue.
...
- Gravitational survey sensors aboard your survey ships generate these points over time.
...
You can assign gravitational survey orders through the Task Group Orders window (see [Section 9 Fleet Management](../9-fleet-management/9.2-fleet-orders.md)):
...
- **Stellar Lagrange Points**: Points defined by the primary star's interaction with the galactic center of mass. These are the most common jump point locations.
- **Planetary Lagrange Points**: Points created by large planets' gravitational influence. Less common but possible.
- **Binary Star Points**: In binary or multiple star systems, the interaction between stellar components creates additional potential jump point locations.

### Number of Survey Locations

Each system has a fixed number of gravitational survey locations, determined at system generation. This number is typically:

- **Small Systems** (few planets): 5-15 survey locations.
- **Medium Systems**: 15-30 survey locations.
- **Large Systems** (many planets, complex orbits): 30-50+ survey locations.

Not every location will contain a jump point. Many (often the majority) will be empty. The ratio of actual jump points to survey locations varies but is typically around 20-40%.
...
- **Jump Drive**: Almost essential for gravitational survey ships (see [Section 8 Ship Design](../8-ship-design/8.1-design-overview.md)), as their primary purpose is finding and transiting jump points. A grav survey ship without a jump drive is limited to systems with jump gates.
...
Some players design ships that carry both geological and gravitational survey sensors. This provides flexibility:

**Advantages**:
- One ship can do both jobs, reducing the number of ships needed.
- Versatile -- can switch between survey types as needed.
- Efficient for small empires with limited shipbuilding capacity.

**Disadvantages**:
- Larger and more expensive than specialized ships.
- Cannot be in two places at once -- surveying one type means not surveying the other.
- For large empires, dedicated ships are more efficient due to parallel operations.
...
### Dealing with Hostile Space

When exploration brings you near hostile forces:

- **Scout Carefully**: Enter new systems cautiously. A passive-sensor-only approach (see [Section 11 Sensors and Detection](../11-sensors-and-detection/11.1-sensor-types.md)) reduces your chances of being detected.
...
Gravitational surveys can also discover anomalies at survey locations. In version 1.10.0 and later, these anomalies may contain Ancient Constructs -- structures left behind by a precursor civilization that provide research bonuses and serve as defenses against Aether Rifts.
...
- [Section 4 Systems and Bodies](../4-systems-and-bodies/4.1-star-systems.md) -- Star system composition and Lagrange points
- [Section 8 Ship Design](../8-ship-design/8.1-design-overview.md) -- Designing gravitational survey ships with jump drives
- [Section 10 Navigation](../10-navigation/10.1-jump-points.md) -- Jump point transit mechanics and jump gates
- [Section 11 Sensors and Detection](../11-sensors-and-detection/11.1-sensor-types.md) -- Passive vs active sensor considerations for exploration
