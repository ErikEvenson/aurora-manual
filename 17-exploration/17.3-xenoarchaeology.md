...
The following installation types may be recovered from ruins: *(unverified)*
...
- **No Repair Required**: Recovered installations are assumed to be operational. Unlike shipwrecks, there is no salvage or repair phase. *(unverified)*
...
- **Cannot Be Duplicated**: Recovered installations are unique artifacts. You cannot build copies unless you have independently researched the underlying technology. *(unverified)*
...
- **Prerequisite Knowledge**: Recovered tech implicitly grants understanding of any prerequisites you lacked. *(unverified)* If you recover an advanced sensor, you are assumed to understand the intermediate sensor technologies even if you never researched them.
...
- **Prerequisite Bypass**: Recovered technology can bypass normal research prerequisites. *(unverified)* You might obtain an advanced sensor without having researched intermediate sensor technologies.
...
As of v2.8.0, artifacts are no longer trade goods. *(unverified)* Instead:

- **Direct stockpile**: Artifacts recovered from ruins are held directly at the population where they were excavated
- **Display location**: Shown at the top of the third column on the population display, below constructs/ruins and above the capability section *(unverified)*
- **Yield Per Recovery**: Each successful artifact recovery yields between 4 and 200 artifacts (4D50) *(unverified)*
...
- **Xenoarchaeology unit extraction**: Xenoarchaeology units assigned to the population continue searching with a two-step success check:
  ...
  2. Secondary check with success chance: `Artifact Reserve / 50,000` *(unverified)*

- **Recovery amount**: Successful secondary recovery yields 4D50 (4-200) artifacts *(unverified)*
...
For TL6 ruins, this means technologies up to 128,000 RP cost can be recovered.\hyperlink{ref-17.3-6}{[6]} Additionally, component development cost can never be lower than build cost. *(unverified)*
...
The survey site list is accessible as a dedicated tab on the Tactical Map window. *(unverified)* This placement integrates site management with ground operations planning.
