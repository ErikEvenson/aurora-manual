...
- Home: Center the map on the primary star *(unverified -- the specific key binding has not been confirmed in available sources)*
...
- Shift+Drag: Measure distance between two points on the tactical map *(unverified -- exact display unit not confirmed; see [Section 3.2.6 Distance Measuring](3.2-system-map.md) for details)*
...
- Up/Down arrows: Move selection between rows in data grids *(unverified)*
- Enter: Confirm selection or perform default action on selected row *(unverified)*
- Tab: Move between fields in data entry areas *(unverified)*
...
- Escape: Close the current window or cancel an operation *(unverified)*
- Ctrl+Z: Undo in some text fields (not universal) *(unverified)*
- Standard Windows shortcuts (Ctrl+C for copy) work in text fields *(unverified)*
