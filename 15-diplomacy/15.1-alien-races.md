...
- **Xenophobia** (1-100): Measures how much the NPR distrusts alien races. Affects diplomatic point generation, territorial responses, and communication willingness. An NPR with Xenophobia of 100 will automatically assign any other race as hostile and cease all communication attempts.
...
- **Response to Threats**: When NPRs detect your ships in their territory, their response depends on their Xenophobia, Militancy, and your diplomatic relationship. See Section 15.1.4 for detailed territory intrusion mechanics.
...
### NPR Ship Design Philosophy (v2.2+)

As of version 2.2, Aurora uses an expanded design philosophy system for NPR ship generation rather than randomly selecting from fixed design themes. Individual aspects of NPR design are selected independently, producing significantly more varied opponents.
...
- **Diplomacy Ships**: Equip ships with Diplomacy Modules (30 HS, 300 BP, 50 crew) to enable effective communication and maximize diplomatic point generation during contact.
...
## 15.1.6 Interrogation of Survivors

Interrogating survivors from destroyed enemy ships generates Race Intelligence Points, providing an alternative intelligence-gathering method to ELINT operations.

### Intelligence Point Generation

The rate of intelligence acquisition from survivors is calculated as:

```
Intelligence Points = (Crew / 10) + (Rank Number of Captured Officer ^ 3)
```

For example, an R3 (rank 3) officer contributes 27 points (3 cubed = 27) in addition to any crew-based points. Higher-ranking captured officers provide exponentially more intelligence.

### Relationship-Based Modifiers

Intelligence gains are reduced based on the political relationship with the defeated power:

| Relationship | Modifier |
|---|---|
| Hostile | Full intelligence gain (no reduction) |
| Neutral | Intelligence points halved (50% reduction) |
...
There is a further 80% reduction if the alien language has not been translated. This stacks with the relationship modifier, making untranslated hostile survivors yield only 20% of base intelligence.

### Integration with Overall Intelligence

Interrogation points contribute to the broader race intelligence system used for espionage and technological discovery, merging with points gained through ELINT operations. At 100 accumulated racial intelligence points, automated espionage checks occur, potentially yielding new technologies, survey data, ship class details, or sensor information.

## 15.1.7 ELINT Intelligence Gathering

Electronic Intelligence (ELINT) modules provide passive intelligence gathering against alien populations and active sensors without requiring combat or prisoner capture.

### Population Intelligence

ELINT modules within range of an alien population gather intelligence on that population at a rate of 1 point per day. This rate is modified by:

- **Commander Intelligence Bonus:** Multiplies the base gathering rate
- **Xenophobia Penalty:** Rate is multiplied by (100 - Target Population Species Xenophobia) / 100
- **Language Penalty:** All intelligence gains are reduced by 80% if the alien language has not been translated

### Intelligence Decay

When monitoring ceases (ELINT module leaves range), accumulated intelligence points decay at approximately 25% per year. This creates incentive to maintain persistent intelligence coverage.

### Population Intelligence Thresholds

As intelligence points accumulate on a specific population, progressively more detailed information becomes available:
...
### Active Sensor Intelligence

ELINT modules also gather data on active sensors they detect, at a rate of 1 point per day per sensor. Initially only sensor strength is displayed. After accumulating 100 points on a specific sensor, its range and resolution become visible.

### Racial Intelligence Conversion

Population intelligence points convert to racial intelligence points on a 1:1 basis, with adjustments for populations under 100 million (smaller populations contribute proportionally less). At 100 accumulated racial intelligence points, an automated espionage check occurs with a chance of yielding actionable intelligence.

### Intelligence Check Categories

When accumulated racial intelligence points reach certain thresholds, automated checks can yield:

- Technology theft (specific technologies from the alien race)
- Missile blueprints
- Gravitational survey data
- Geological survey data
- New system locations
- Ship class summaries
...
Minor Races are a reduced-capability NPC faction type introduced in version 2.4. They represent small civilizations that lack the ambition or capability for interstellar expansion.
