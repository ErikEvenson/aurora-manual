...
![](../images/art/section-art/15.2.png)

> **[Screenshot Pending — #1083]** Diplomacy Window Communication Tab
...
![Communication Rating Impact Levels](../images/diagrams/15-diplomacy/15.2-communication-rating-impact.png)
...
![Translation Progress Timeline](../images/diagrams/15-diplomacy/15.2-translation-timeline.png)
...
### 15.2.2.4 Communication Framework

*Added: v2.8.0*

The communication system uses a structured framework to determine translation progress. Understanding this framework helps optimize diplomatic ship deployment and commander assignments. \hyperlink{ref-15.2-4}{[4]}

#### Contact Rating Calculation

The contact rating determines how effectively both races can make translation progress. It is calculated based on the following conditions:

| Condition | Contact Rating |
|-----------|---------------|
| Both races detect each other + diplomacy ship + commander bonus | 1 + commander bonus |
| Both races detect each other + diplomacy ship (no commander bonus) | 1.0 |
| Both races detect each other (no diplomacy ship) | 0.5 |
| Either race cannot detect the other | 0 |

**Prisoner Processing Bonus**: If you have captured prisoners from the target race, processing them adds to the contact rating:
- Bonus = processed prisoners / 200
- Maximum bonus from prisoners: +1.0

This means that even without a diplomatic ship, processing 200+ prisoners can provide the equivalent of a full commander bonus for translation progress.

#### Prisoner Processing and Communication

*Added: v2.8.0*

Prisoner processing operates differently depending on whether communication has been established with the prisoner's race: \hyperlink{ref-15.2-5}{[5]}

**Pre-Communication Processing:**
- Up to 200 prisoners per colony are processed to aid communication attempts
- These prisoners contribute to the Contact Rating formula above
- **No intelligence points** are generated from pre-communication processing
- This represents linguistic analysis and cultural study rather than interrogation

**Post-Communication Processing:**
- Once communication is established, remaining prisoners are processed normally
- Standard interrogation rules apply, generating Race Intelligence Points
- Intelligence gain formula: `Crew / 10 + (Rank Number)³` for any captured officers

**Strategic Implications:**
- Capturing alien crew early in first contact accelerates communication establishment
- The 200-prisoner threshold per colony means distributing prisoners across multiple colonies does not increase the communication bonus (each colony caps at +1.0)
- Prioritize establishing communication before processing large numbers of prisoners if intelligence gathering is your goal

#### Communication Roll Formula

Each communication attempt uses the following formula to determine progress:

```
Result = average translation skill + d100 + modifiers
```

**Modifiers:**
| Condition | Modifier |
|-----------|----------|
| Same species (e.g., human [NPR](../appendices/B-glossary.md#npr)) | +20 |
| Spoiler races | -50 |

**Roll Outcomes:**

The roll result determines whether communication improves, degrades, or reaches a terminal state:

| Result | Outcome |
|--------|---------|
| >= 100 | Full communication established immediately |
| Positive | Communication rating improves |
| Negative | Communication rating decreases |
| <= -50 | Communication permanently impossible |

**Critical Result**: A roll result of 100 or higher immediately establishes full communication, bypassing the gradual progress system. This makes high-skill commanders valuable even early in first contact scenarios.

**Permanent Failure**: A roll result of -50 or lower permanently locks the communication status to "impossible." This typically occurs when attempting communication with spoiler races (which have a -50 modifier) combined with low translation skill and an unlucky roll.

#### Progress Notifications

The game provides feedback on translation progress through event notifications:

| Notification | Condition |
|--------------|-----------|
| "significant progress" | Roll modification >= 10 |
| "glacial progress" | Positive modification < 10 |
| "communication attempts have failed" | Roll modification < -3 |

**Note**: "Glacial progress" notifications were added in v2.8.0 to provide feedback when translation is advancing slowly but still positively.

#### Strategic Implications

The communication framework rewards:

1. **High-skill Commanders**: The translation skill directly adds to the roll, making skilled diplomats significantly more effective
2. **Diplomatic Ship Presence**: Without a diplomacy module in the contact system, the contact rating is halved (0.5 vs 1.0+)
3. **Prisoner Acquisition**: Processing prisoners provides a substantial bonus that stacks with commander skill
4. **Same-Species Contact**: Human NPRs are 20 points easier to communicate with, making them significantly more likely to achieve breakthrough communication quickly

**Warning**: Avoid early communication attempts with spoiler races unless your translation skill is exceptionally high. The -50 modifier combined with a poor roll can permanently lock communication status before you have a chance to improve your capabilities.
...
- **Default Off**: NPR military ships generally operate with transponders off, requiring player sensors to detect them *(unverified — [#705](https://github.com/ErikEvenson/aurora-manual/issues/705))*
- **Civilian Traffic**: NPR civilian shipping may broadcast transponders to friendly races, similar to player civilian ships *(unverified — [#705](https://github.com/ErikEvenson/aurora-manual/issues/705))*
...
- **Size**: 30 [HS](../appendices/B-glossary.md#hs) (Hull Spaces) (see [Section 8.1 Design Philosophy](../8-ship-design/8.1-design-philosophy.md))
- **Cost**: 300 [BP](../appendices/B-glossary.md#bp) (Build Points)
- **Crew**: 50
...
- **Availability**: Starting technology for both [TN](../appendices/B-glossary.md#tn) (Trans-Newtonian) and conventional starts
...
For boarding combat mechanics, see [Section 13.3 Ground Combat](../13-ground-forces/13.3-ground-combat.md).
...
\hypertarget{ref-15.2-4}{[4]}. Aurora Forums -- Steve Walmsley post, January 31, 2026 -- Communication framework mechanics: contact rating calculation, communication roll formula, and notification tiers

\hypertarget{ref-15.2-5}{[5]}. Aurora Forums -- Steve Walmsley post, February 2, 2026 -- v2.8.0 Changes List thread: Pre-communication prisoner processing (up to 200 per colony) aids communication attempts without generating intelligence points
